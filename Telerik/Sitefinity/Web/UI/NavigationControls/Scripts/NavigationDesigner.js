...
Telerik.Sitefinity.Web.UI.NavigationControls.NavigationDesigner=function(element){this._pageSelectedDelegate=null;
...
Telerik.Sitefinity.Web.UI.NavigationControls.NavigationDesigner.prototype={initialize:function(){Telerik.Sitefinity.Web.UI.NavigationControls.NavigationDesigner.callBaseMethod(this,"initialize");
...
this._toogleDesignSettingsDelegate=Function.createDelegate(this,function(){$("#groupDesignSettings").toggleClass("sfExpandedSection");
...
},dispose:function(){Telerik.Sitefinity.Web.UI.NavigationControls.NavigationDesigner.callBaseMethod(this,"dispose");
...
this._headingsModeDelegate=null;
this._toogleDesignSettingsDelegate=null;
if(this.get_customSelectedPagesControl()){this.get_customSelectedPagesControl().remove_itemsChanged(this._pageReorderedDelegate);
delete this._pageReorderedDelegate;
}},get_parentPageCheckBox:function(){var mode=this.get_controlData().SelectionMode;
var parentPageCheckBox;
switch(mode){case"SelectedPageChildren":case"CurrentPageChildren":case"CurrentPageSiblings":parentPageCheckBox=jQuery(".sfShowParentPage",this.get_element()).filter("[id*='"+mode+"']").get(0);
break;
default:parentPageCheckBox=null;
break;
}return parentPageCheckBox;
},get_groupPageNavigationCheckBox:function(){return jQuery(this.get_element()).find("#cbGroupPageNavigation").get(0);
...
},get_currentPageSelectZone:function(){if(this._currentPageSelectZone==null){this._currentPageSelectZone=this.findElement("currentPageSelectionZone");
}return this._currentPageSelectZone;
},get_currentPageSiblingsZone:function(){if(this._currentPageSiblingsZone==null){this._currentPageSiblingsZone=this.findElement("currentPageSiblingsZone");
...
},get_multiSelectorTag:function(){if(this._multiSelectorTag==null){this._multiSelectorTag=this.findElement("multiPageSelectorTag");
}return this._multiSelectorTag;
},get_customPagesSelector:function(){if(this._customPagesSelector==null){this._customPagesSelector=this.findElement("liCustomSelection");
}return this._customPagesSelector;
},get_expandLevelsTextBox:function(){if(this._expandLevelsTextBox==null){this._expandLevelsTextBox=this.findElement("txtExpandLevels");
}return this._expandLevelsTextBox;
},get_bindingLevelsTextBox:function(){if(this._bindingLevelsTextBox==null){this._bindingLevelsTextBox=this.findElement("txtBindingLevels");
}return this._bindingLevelsTextBox;
},get_headingsCheckBox:function(){if(this._headingsCheckBox==null){this._headingsCheckBox=this.findElement("cbHeadings");
...
},get_customTemplateTextBox:function(){if(this._customTemplateTextBox==null){this._customTemplateTextBox=this.findElement("txtCustomTemplatePath");
}return this._customTemplateTextBox;
},get_skinTextBox:function(){if(this._skinTextBox==null){this._skinTextBox=this.findElement("txtSkin");
}return this._skinTextBox;
},get_allowCollapsingCheckBox:function(){if(this._allowCollapsingCheckBox==null){this._allowCollapsingCheckBox=this.findElement("cbAllowCollapsing");
}return this._allowCollapsingCheckBox;
},get_expandAllCheckBox:function(){if(this._expandAllCheckBox==null){this._expandAllCheckBox=this.findElement("cbExpandAllLevels");
}return this._expandAllCheckBox;
},get_selectionHeadingLabels:function(){return this._selectionHeadingLabels;
},set_selectionHeadingLabels:function(value){this._selectionHeadingLabels=value;
...
this._selectedPagesArrayChanged=false;
if(this.get_parentPageCheckBox()){this.get_parentPageCheckBox().checked=data.ShowParentPage;
...
if(data.LevelsToExpand>0){this._clickRadioChoice("LevelsToExpandMode","SpecifyLevels");
jQuery(this.get_expandLevelsTextBox()).val(data.LevelsToExpand);
}else{this._clickRadioChoice("LevelsToExpandMode","AllLevels");
}if(data.MaxDataBindDepth>0){this._clickRadioChoice("MaxDataBindDepthMode","SpecifyBindingLevels");
jQuery(this.get_bindingLevelsTextBox()).val(data.MaxDataBindDepth);
}else{this._clickRadioChoice("MaxDataBindDepthMode","AllBindingLevels");
...
}if(this.get_controlData().Skin!=null){this.get_skinTextBox().value=this.get_controlData().Skin;
...
this._refreshMode=false;
},applyChanges:function(){var txtVal=this.get_expandLevelsTextBox().value;
if(txtVal==""){txtVal="0";
...
if(this.get_parentPageCheckBox()){controlData.ShowParentPage=this.get_parentPageCheckBox().checked;
}var groupPageNavigationCheckBox=this.get_groupPageNavigationCheckBox();
if(groupPageNavigationCheckBox){controlData.GroupPageIsLink=groupPageNavigationCheckBox.checked;
...
},_hideAllSettings:function(){jQuery(this.get_element()).find("#NavigationSettings div[id^='groupSetting']").hide();
},_showSettingGroup:function(settingGroupName){jQuery(this.get_element()).find("#NavigationSettings #"+settingGroupName).show();
...
dialogBase.resizeToContent();
},_showMultiPageSelector:function(){this._pagesDialog.dialog("open");
jQuery("body > form").hide();
dialogBase.resizeToContent();
},_updateCustomTemplate:function(){if(this._currentNavigationMode==null){return;
}var templateValue=this.get_customTemplateTextBox().value;
var data=this.get_controlData();
data["CustomTemplate"+this._currentNavigationMode]=templateValue;
},_removeClickHandlersManipulatingGroupCheckBox:function(){jQuery(this.get_element()).undelegate("#rbMouseOver","click");
jQuery(this.get_element()).undelegate("#rbClick","click");
jQuery(this.get_element()).undelegate("#cbExpandAllLevels","click");
},_setupGroupCheckBox:function(){var groupPageNavigationCheckBox=this.get_groupPageNavigationCheckBox();
if(groupPageNavigationCheckBox){groupPageNavigationCheckBox.checked=this.get_controlData().GroupPageIsLink;
...
}},_addClickHandlersToManipulateGroupCheckBox:function(navigationOption){var that=this;
var groupPageNavigationCheckBox=this.get_groupPageNavigationCheckBox();
switch(navigationOption){case"HorizontalDropDownMenu":case"HorizontalTabs":var mouseClickOpenOption=jQuery(this.get_element()).find("#rbClick").get(0);
if(mouseClickOpenOption){if(mouseClickOpenOption.checked){that._disableGroupNavigationOnClick(groupPageNavigationCheckBox);
}}jQuery(this.get_element()).delegate("#rbMouseOver","click",function(e){that._enableGroupPageNavigationOnMouseOver(groupPageNavigationCheckBox);
});
jQuery(this.get_element()).delegate("#rbClick","click",function(e){that._disableGroupNavigationOnClick(groupPageNavigationCheckBox);
});
break;
case"VerticalTree":var showExpandedNodesElement=jQuery(this.get_element()).find("#cbExpandAllLevels").get(0);
if(showExpandedNodesElement){that._changeGroupPageNavigationOnShowExpandedNodes(showExpandedNodesElement,groupPageNavigationCheckBox);
}jQuery(this.get_element()).delegate("#cbExpandAllLevels","click",function(e){that._changeGroupPageNavigationOnShowExpandedNodes(this,groupPageNavigationCheckBox);
});
break;
}},_clickedNavigationMode:function(sender){var data=this.get_controlData();
...
if(!this._refreshMode){data.NavigationMode=sender.target.value;
this._updateCustomTemplate();
}if(data.NavigationMode!="VerticalSimple"){if(data.SelectionMode=="CurrentPageSiblings"){this._clickRadioChoice("SelectionMode","TopLevelPages");
}}if(data.SelectionMode=="SelectedPages"&&data.NavigationMode!="VerticalSimple"&&data.NavigationMode!="HorizontalSimple"){this._clickRadioChoice("SelectionMode","TopLevelPages");
}this._currentNavigationMode=sender.target.value;
if(data["CustomTemplate"+this._currentNavigationMode]!=null){this.get_customTemplateTextBox().value=data["CustomTemplate"+this._currentNavigationMode];
}else{this.get_customTemplateTextBox().value="";
}},_enableGroupPageNavigationOnMouseOver:function(groupPageNavigationCheckBox){var disabledGroupPageNavigation=jQuery(this.get_element()).find(".sfGroupPageNavigationDisabled");
if(disabledGroupPageNavigation){disabledGroupPageNavigation.removeClass("sfGroupPageNavigationDisabled");
}groupPageNavigationCheckBox.disabled=false;
},_disableGroupNavigationOnClick:function(groupPageNavigationCheckBox){var groupPageNavigationWrapper=jQuery(this.get_element()).find(".sfGroupPageNavigationWrapper");
if(groupPageNavigationWrapper){groupPageNavigationWrapper.addClass("sfGroupPageNavigationDisabled");
}groupPageNavigationCheckBox.checked=false;
groupPageNavigationCheckBox.disabled=true;
},_changeGroupPageNavigationOnShowExpandedNodes:function(showExpandedNodesElement,groupPageNavigationCheckBox){var groupPageNavigationWrapper=jQuery(this.get_element()).find(".sfGroupPageNavigationWrapper");
if(groupPageNavigationWrapper&&showExpandedNodesElement.checked){groupPageNavigationWrapper.addClass("sfGroupPageNavigationDisabled");
groupPageNavigationCheckBox.checked=true;
groupPageNavigationCheckBox.disabled=true;
}else{groupPageNavigationWrapper.removeClass("sfGroupPageNavigationDisabled");
groupPageNavigationCheckBox.disabled=false;
...
}},_clickedNavigationAction:function(sender){if(!this._refreshMode){this.get_controlData().NavigationAction=sender.target.value;
}},_clickedLevelsToExpandMode:function(sender){if(sender.target.value=="AllLevels"){jQuery(this.get_expandLevelsTextBox()).attr("disabled","true");
if(!this._refreshMode){jQuery(this.get_expandLevelsTextBox()).val("");
this.get_controlData().ExpandLevel=0;
}}else{jQuery(this.get_expandLevelsTextBox()).val("1");
jQuery(this.get_expandLevelsTextBox()).attr("disabled",null);
}},_clickedMaxDataBindDepthMode:function(sender){if(sender.target.value=="AllBindingLevels"){jQuery(this.get_bindingLevelsTextBox()).attr("disabled","true");
if(!this._refreshMode){this.get_controlData().MaxDataBindDepth=0;
jQuery(this.get_bindingLevelsTextBox()).val("");
}}else{jQuery(this.get_bindingLevelsTextBox()).val("2");
jQuery(this.get_bindingLevelsTextBox()).attr("disabled",null);
}},_clickedHeadingsMode:function(sender){if(!this._refreshMode){this.get_controlData().HeadingsShouldBeLinks=sender.target.checked;
}},_clickedExpandAllCheckBox:function(sender){if(!this._refreshMode){if(sender.target.checked==true){this.get_controlData().LevelsToExpand=0;
this.get_allowCollapsingCheckBox().disabled=null;
this.get_expandLevelsTextBox().value="";
}else{this.get_controlData().LevelsToExpand=1;
...
}}},_showTreeViewExpandSettings:function(){var levels=this.get_controlData().LevelsToExpand;
if(levels==0){this.get_expandAllCheckBox().checked=true;
this.get_allowCollapsingCheckBox().disabled=null;
}if(levels>0){this.get_expandAllCheckBox().checked=false;
this.get_allowCollapsingCheckBox().disabled=true;
}},_showPageSelectors:function(value){jQuery([this.get_pageSelectZone(),this.get_multiPageSelectZone(),this.get_currentPageSiblingsZone(),this.get_currentPageSelectZone()]).hide();
...
break;
case"CurrentPageChildren":jQuery(this.get_currentPageSelectZone()).show();
break;
case"CurrentPageSiblings":jQuery(this.get_currentPageSiblingsZone()).show();
break;
case"CustomSelection":break;
}dialogBase.resizeToContent();
},_showRelevantSettings:function(navOption){this._hideAllSettings();
...
multiplePages.hide();
if(!this._refreshMode){this.applyChanges();
}switch(navOption){case"HorizontalSimple":this._showSettingGroup("groupSettingPageSelect");
multiplePages.show();
break;
case"HorizontalDropDownMenu":this._showSettingGroup("groupSettingPageSelect");
...
break;
case"HorizontalTabs":this._showSettingGroup("groupSettingPageSelect");
this._showSettingGroup("groupSettingBinding");
this._showSettingGroup("groupSettingDropDown");
this._showSettingGroup("groupSettingGroupPageNavigation");
break;
case"VerticalSimple":this._showSettingGroup("groupSettingPageSelect");
multiplePages.show();
siblingsSelector.show();
showParentPagesCheckBoxes.show();
break;
case"VerticalTree":this._showSettingGroup("groupSettingPageSelect");
this._showSettingGroup("groupSettingBinding");
...
this._showSettingGroup("groupSettingGroupPageNavigation");
break;
case"SiteMapInColumns":this._showSettingGroup("groupSettingPageSelect");
this._showSettingGroup("groupSettingGroupPageNavigation");
break;
case"SiteMapInRows":this._showSettingGroup("groupSettingPageSelect");
this._showSettingGroup("groupSettingGroupPageNavigation");
break;
case"CustomNavigation":this._showSettingGroup("groupSettingPageSelect");
this._showSettingGroup("groupSettingGroupPageNavigation");
break;
