...
Telerik.Sitefinity.Modules.Newsletters.Web.UI.Forms.ManageSubscribersDialog = function (element) {
...
Telerik.Sitefinity.Modules.Newsletters.Web.UI.Forms.ManageSubscribersDialog.prototype = {

    // set up 
    initialize: function () {
        manageSubscribersDialog = this;
    ...
    loadDialog: function (listId, listTitle) {
        this._mailingListId = listId;
    ...
    _getSubscribersByMailingList: function () {
        var webServiceUrl = this._serviceBaseUrl + "mailingList/" + this._mailingListId;
    ...
    _getSubscribers_Success: function (sender, args) {
        var items = args.Items;
        for (var i = 0; i < items.length; i++) {
            sender._originallySelectedIds.push(items[i].Id);
        }
        if (!sender._subscribersSelectorBinded) {
            sender._subscribersSelectorBinded = true;
            sender._subscribersSelector.dataBind();
        }
        else {
            sender._subscribersSelector.selectItemsInternal(sender._originallySelectedIds);
        }
    },

    _getSubscribers_Failure: function (sender, args) {
        alert('Failure!');
    },

    _saveChanges: function (sender, args) {
        var webServiceUrl = this._serviceBaseUrl + "add/" + this._mailingListId;
        var clientManager = new Telerik.Sitefinity.Data.ClientManager();
        var urlParams = [];
        var keys = [];
    ...
    _saveChanges_Success: function (sender, args) {
        var context = { ListId: sender._mailingListId };
    ...
    _binderDataBound: function (sender, args) {
        this._subscribersSelector.selectItemsInternal(this._originallySelectedIds);
    },
    ...
    _rowSelected: function (sender, args) {
        ...
        if (target) {
            var subscriber = args.get_gridDataItem().get_dataItem();
            if (this._originallySelectedIds.indexOf(subscriber.Id) == -1) {
                this._originallySelectedIds.push(subscriber.Id);
            }
        }
    },
    ...
    _rowDeselected: function (sender, args) {
        ...
        if (target) {
            ...
            if (idx > -1) {
                this._originallySelectedIds.splice(idx, 1);
            }
        }
    },
    ...
    get_subscribersSelector: function () {
        return this._subscribersSelector;
    },
    set_subscribersSelector: function (value) {
        this._subscribersSelector = value;
    }
};
