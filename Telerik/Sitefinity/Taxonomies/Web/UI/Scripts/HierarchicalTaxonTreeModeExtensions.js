function HierarchicalTaxonTreeModeExtensions_ViewLoaded(masterGridView){var context=this;
this.itemsGrid=null;
...
this.initialize=function(){this._itemsGrid_ItemCommandDelegate=Function.createDelegate(this,this.itemsGrid_ItemCommand);
...
if(createNewWidget){var createText=createNewWidget.get_buttonText();
...
if(moreActionsWidget){var moreActionElements=moreActionsWidget._menu.get_allItems();
var moreActionsIter=moreActionElements.length;
while(moreActionsIter--){var action=moreActionElements[moreActionsIter];
if(action.get_value()=="batchChangeParent"){var txt=action.get_text();
txt=this.GetFormattedString(txt,this.singleTaxonName.htmlEncode());
action.set_text(txt);
}}}};
this.selectionChanged=function(sender,args){masterGridView._setToolbarButtonsEnabledState();
};
this.itemsGrid_binder_itemDataBound=function(sender,args){var container=args.get_itemElement();
var dataItem=args.get_dataItem();
var self=this;
$(container).find(".sf_binderCommand_changeParent").each(function(){var html=$(this).html();
html=String.format(html,self.singleTaxonName.toLowerCase());
$(this).text(html);
});
var itemsCount=dataItem.ItemsCount;
if(itemsCount==0){var oThis=this;
$(container).find(".sf_binderCommand_viewMarkedItems").each(function(){var div=$(this).parent();
div.html(itemsCount+" "+oThis.clientLabelManager.getLabel("Labels","Items"));
});
}};
this.itemsGrid_Command=function(sender,args){var commandName=args.get_commandName();
switch(commandName){case"batchMoveUp":if(sender.getBinder().get_selectedItemsCount()>0){args.set_cancel(true);
this.moveTaxons("up");
}break;
case"batchMoveDown":if(sender.getBinder().get_selectedItemsCount()>0){args.set_cancel(true);
this.moveTaxons("down");
}break;
case"batchChangeParent":if(sender.getBinder().get_selectedItemsCount()>0){args.set_cancel(true);
this.itemsGrid.openDialog("changeParent",this.itemsGrid.get_selectedItems(),{TaxonomyId:this.taxonomyId,TaxonName:this.singleTaxonName,TaxonomyTitle:this.taxonomyName,UiCulture:this.itemsGrid.get_uiCulture()});
}break;
default:args.set_cancel(false);
break;
}};
this.itemsGrid_ItemCommand=function(sender,args){var commandName=args.get_commandName();
var dataItem=args.get_commandArgument();
switch(commandName){case"changeParent":args.set_cancel(true);
this.itemsGrid.openDialog("changeParent",[dataItem],{TaxonomyId:this.taxonomyId,TaxonName:this.singleTaxonName,TaxonomyTitle:this.taxonomyName,UiCulture:this.itemsGrid.get_uiCulture()});
break;
case"moveUp":args.set_cancel(true);
this.moveTaxon("up",dataItem);
break;
case"moveDown":args.set_cancel(true);
this.moveTaxon("down",dataItem);
break;
default:args.set_cancel(false);
break;
}};
this.moveTaxon=function(direction,dataItem){var baseUrl;
if(direction=="up"){baseUrl=this.moveItemUpServiceUrl;
}else{if(direction=="down"){baseUrl=this.moveItemDownServiceUrl;
...
this.moveTaxon_Success=function(result){this.itemsGrid.getBinder().DataBind(this.expandedParentId);
};
this.moveTaxon_Failure=function(result){alert(result.Detail);
};
this.moveTaxons=function(direction){var baseUrl;
if(direction=="up"){baseUrl=this.batchMoveItemsUpServiceUrl;
}else{if(direction=="down"){baseUrl=this.batchMoveItemsDownServiceUrl;
}else{alert('Unknown direction for moving items "'+direction+'". Supported are "up" and "down".');
return;
...
this.moveTaxons_Success=function(result){this.itemsGrid.getBinder().DataBind();
};
this.moveTaxons_Failure=function(result){alert(result.Detail);
};
this.GetSelectedIds=function(){var selectedIds=[];
var selectedItems=this.itemsGrid.get_selectedItems();
for(var i=0,selectedItemsLength=selectedItems.length;
i<selectedItemsLength;
i++){selectedIds.push(selectedItems[i].Id);
}return selectedIds;
};
this.GetFormattedString=function(str,strToFormat){if(str.indexOf("{0}")!=-1){return String.format(str,strToFormat);
}return str;
};
this.initialize();
}