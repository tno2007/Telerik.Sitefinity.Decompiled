// Decompiled with JetBrains decompiler
// Type: Telerik.Sitefinity.Web.UI.Fields.Definitions.FileFieldDefinition
// Assembly: Telerik.Sitefinity, Version=13.3.7600.0, Culture=neutral, PublicKeyToken=b28c218413bdf563
// MVID: 39C4B52A-B559-4D9C-97D9-CCCF73C3738E
// Assembly location: C:\Programs\Sitefinity\ProjectManager_13_3_7600\_EmptyProject\bin\Telerik.Sitefinity.dll

using System;
using Telerik.Sitefinity.Configuration;
using Telerik.Sitefinity.Web.UI.Definitions;
using Telerik.Sitefinity.Web.UI.Fields.Contracts;

namespace Telerik.Sitefinity.Web.UI.Fields.Definitions
{
  /// <summary>
  /// A definition class containing all information needed to construct an instance of a
  /// <see cref="T:Telerik.Sitefinity.Web.UI.Fields.FileField" />
  /// </summary>
  public class FileFieldDefinition : 
    FieldControlDefinition,
    IFileFieldDefinition,
    IFieldControlDefinition,
    IFieldDefinition,
    IDefinition
  {
    private string itemName;
    private string itemNamePlural;
    private Type libraryContentType;
    private bool isMultiselect;
    private int maxFileCount;

    /// <summary>
    /// Initializes a new instance of the <see cref="T:Telerik.Sitefinity.Web.UI.Fields.Definitions.FileFieldDefinition" /> class.
    /// </summary>
    public FileFieldDefinition()
    {
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="T:Telerik.Sitefinity.Web.UI.Fields.Definitions.FileFieldDefinition" /> class.
    /// </summary>
    /// <param name="configDefinition">The config definition.</param>
    public FileFieldDefinition(ConfigElement configDefinition)
      : base(configDefinition)
    {
    }

    /// <summary>
    /// Gets or sets the name of the single item being managed by the file field control.
    /// </summary>
    /// <value></value>
    public string ItemName
    {
      get => this.ResolveProperty<string>(nameof (ItemName), this.itemName);
      set => this.itemName = value;
    }

    /// <summary>
    /// Gets or sets the name of the single item in plural being managed by the file field control.
    /// </summary>
    /// <value></value>
    public string ItemNamePlural
    {
      get => this.ResolveProperty<string>(nameof (ItemNamePlural), this.itemNamePlural);
      set => this.itemNamePlural = value;
    }

    /// <summary>Gets or sets the type of the library content.</summary>
    /// <value>The type of the library content.</value>
    public Type LibraryContentType
    {
      get => this.ResolveProperty<Type>(nameof (LibraryContentType), this.libraryContentType);
      set => this.libraryContentType = value;
    }

    /// <summary>
    /// Gets or sets a value indicating whether the uploader allows multiple files to be selected.
    /// </summary>
    /// <value></value>
    public bool IsMultiselect
    {
      get => this.ResolveProperty<bool>(nameof (IsMultiselect), this.isMultiselect);
      set => this.IsMultiselect = value;
    }

    /// <summary>
    /// Gets or sets the maximum allowed number of files selected in the uploader.
    /// </summary>
    /// <value></value>
    public int MaxFileCount
    {
      get => this.ResolveProperty<int>(nameof (MaxFileCount), this.maxFileCount);
      set => this.MaxFileCount = value;
    }
  }
}
