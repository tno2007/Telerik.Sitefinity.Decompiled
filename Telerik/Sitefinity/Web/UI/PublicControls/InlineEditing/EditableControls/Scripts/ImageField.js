define(["EditableWindowField","DetailViewEditingWindow","text!ImageFieldTemplate!strip","ImageSelector"],function(EditableWindowField,DetailViewEditingWindow,html,ImageSelector){function ImageField(options){var that=this;
...
},dateString:function(e){var wcfDate=this.dataItem.DateCreated;
if(wcfDate.match){return new Date(wcfDate.match(/\d+/)[0]*1).format(that.dateFormat);
}return wcfDate.format(that.dateFormat);
}});
return(this);
}ImageField.prototype={success:function(data,textStatus,jqXHR){this.editWindowViewModel.set("dataItem",data.Image);
this.detailsViewInfo=data.DetailsViewInfo;
this.originalValue=data.Image;
if(!this.isInitialized){kendo.bind(this.dialog.getContentPlaceHolder(),this.editWindowViewModel);
...
},isChanged:function(){if(!this.value){return false;
}var isNewUploadedItem=this.value.Status=="Master";
if(isNewUploadedItem){return true;
}var isNewSelectedItem=(this.originalValue.Id!==this.value.Id)||(this.originalValue.ProviderName!==this.value.ProviderName);
if(isNewSelectedItem){return true;
}var isCurrentItemChanged=(this.originalValue.Title!==this.value.Title)||(this.originalValue.AlternativeText!==this.value.AlternativeText);
if(isCurrentItemChanged){return true;
}return false;
},openImageInBackend:function(e){var data={ShowMoreActionsWorkflowMenu:false,HideLanguageList:true,DetailsViewUrl:this.detailsViewInfo.DetailsViewUrl,ItemId:this.detailsViewInfo.ItemId,ItemType:this.detailsViewInfo.ItemType,ProviderName:this.detailsViewInfo.ProviderName,CommandName:this.detailsViewInfo.CommandName,Params:{IsEditable:true,parentId:this.detailsViewInfo.ParentId,parentType:this.detailsViewInfo.ParentType},Culture:this.contentRepository.culture};
...
var that=this;
this.contentRepository.getLiveImageData(parentId,originalContentId,this.detailsViewInfo.ProviderName,function(data,textStatus,jqXHR){that.onImageSelected(null,[data]);
});
...
}if(selectedFile){this.editWindowViewModel.dataItem.Id=selectedFile.LiveContentId;
...
}},onDone:function(event,sender){var imageItem=[{Name:this.fieldName,Value:null,ComplexValue:[{Name:"Title",Value:this.editWindowViewModel.dataItem.Title},{Name:"AlternativeText",Value:this.editWindowViewModel.dataItem.AlternativeText},{Name:"Id",Value:this.editWindowViewModel.dataItem.Id},{Name:"ProviderName",Value:this.editWindowViewModel.dataItem.ProviderName}]}];
this.value=jQuery.extend(true,{},this.editWindowViewModel.dataItem);
