...
Telerik.Sitefinity.Web.UI.Selectors.ExternalPagesSelector=function(element){Telerik.Sitefinity.Web.UI.Selectors.ExternalPagesSelector.initializeBase(this,[element]);
...
Telerik.Sitefinity.Web.UI.Selectors.ExternalPagesSelector.prototype={initialize:function(){Telerik.Sitefinity.Web.UI.Selectors.ExternalPagesSelector.callBaseMethod(this,"initialize");
...
},dispose:function(){if(this._urlFocusDelegate){if(this._urlTextBox){$removeHandler(this._urlTextBox.get_textElement(),"click",this._urlFocusDelegate);
}delete this._urlFocusDelegate;
}if(this._pageClickedDelegate){delete this._pageClickedDelegate;
...
},get_selectedItems:function(){return this.get_itemsBuilder().getSelectedPages();
},set_selectedItems:function(items){if(!items||items.length==0){this._showHideNoSelectionLabel(true);
...
},_pageLoadHandler:function(){this._refreshEditSection();
},_saveChangesHandler:function(){var titleTextBox=this.get_titleTextBox();
...
},_addButtonHandler:function(){var validInput=this._isInputValid()&&this.get_urlTextBox().validate();
if(validInput==true){var titleTextBox=this.get_titleTextBox();
var urlTextBox=this.get_urlTextBox();
...
}else{}},_pageClickedHandler:function(sender,args){var index=args.Index;
var page=args.Item;
...
},_itemsChangedHandler:function(sender,args){this._showHideNoSelectionLabel(args.ItemsCount==0);
},_urlFocusHandler:function(e){},_fieldKeyPressHandler:function(){this._refreshEditSection();
},_isInputValid:function(){var titleTextbox=this.get_titleTextBox();
var urlTextbox=this.get_urlTextBox();
if(jQuery.trim(titleTextbox.value).length>0&&jQuery.trim(urlTextbox.get_value()).length>0){return true;
}else{return false;
}},_refreshEditSection:function(){var validInput=this._isInputValid();
if(validInput==true){this._enableEditSection();
}else{this._disableEditSection();
}},_disableEditSection:function(){this.get_saveChangesButton().disabled=true;
},_enableEditSection:function(){this.get_saveChangesButton().disabled=false;
...
return result;
},_showHideNoSelectionLabel:function(val){if(val){jQuery(this.get_noPagesSelectedLabel()).show();
}else{jQuery(this.get_noPagesSelectedLabel()).hide();
}},get_saveChangesButton:function(){if(this._saveChangesButton==null){this._saveChangesButton=this._findElement("addButton");
}return this._saveChangesButton;
},get_titleTextBox:function(){if(this._titleTextBox==null){this._titleTextBox=this._findElement("titleTextBox");
}return this._titleTextBox;
},get_urlTextBox:function(){return this._urlTextBox;
},set_urlTextBox:function(value){this._urlTextBox=value;
},get_openInNewWindowChoiceField:function(){return this._openInNewWindowChoiceField;
},set_openInNewWindowChoiceField:function(value){this._openInNewWindowChoiceField=value;
},get_noPagesSelectedLabel:function(){if(this._noPagesSelectedLabel==null){this._noPagesSelectedLabel=this._findElement("noPagesSelectedLabel");
