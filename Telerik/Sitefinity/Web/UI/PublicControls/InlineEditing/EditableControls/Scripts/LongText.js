define(["EditableField","ImageSelector","ImageEditor","HyperLinkManager","DialogBase"],function(EditableField,ImageSelector,ImageEditor,HyperLinkManager,DialogBase){function EditableLongText(options){EditableField.call(this,options);
...
return(this);
}EditableLongText.prototype={showEditMode:function(){if(!this.isInEdit){var that=this;
var isMoved=false;
this.element.attr("contenteditable",true);
this.editor=this.element.kendoEditor({tools:["formatting","bold","italic","underline","justifyLeft","justifyCenter","justifyRight","justifyFull","insertOrderedList","insertUnorderedList","indent","outdent",{name:"insertImage",tooltip:"Insert image",exec:function(e){e.preventDefault();
e.stopPropagation();
var editor=$(this).data("kendoEditor");
that.editorRange=editor.getRange();
that.imgTag=$(editor.selectedHtml());
if(that.imgTag.is("img")){that.imageEditor.refresh(that.imgTag);
that.imageEditorDialog.open();
}else{that.imageSelector.show();
}}},{name:"createLink",tooltip:"Select link",exec:function(e){e.preventDefault();
e.stopPropagation();
...
}},"unlink","createTable","addRowAbove","addRowBelow","addColumnLeft","addColumnRight","deleteRow","deleteColumn","viewHtml"],execute:function(e){if(e.name==="viewhtml"){$(that.editor.element).children().filter(function(index,item){return $(item).attr("data-sf-ignore")!==undefined;
}).remove();
}}}).data("kendoEditor");
this.editorWindow=this.editor.toolbar.element.parent().data("kendoWindow");
this.editorWindow.setOptions({animation:{close:false,open:false}});
this.editorWindow.bind("close",function(e){$(document.body).removeClass("sfEditorToolbarShown");
if(isMoved){$(document.body).animate({"margin-top":"-=45"},200);
isMoved=false;
}});
this.editorWindow.bind("open",function(e){that.editorWindow.element.parent().css({top:43,position:"fixed",width:"100%",padding:0,border:0,left:0,display:""});
$(document.body).addClass("sfEditorToolbarShown");
if(that.element.position().top<89){isMoved=true;
...
if(draggableElement!==undefined){draggableElement.destroy();
...
$(this.imageEditorDialog).on("closeSelected",function(event,sender){sender.close();
});
if(this.editor){$(this.editor.body).focusin();
}}},unselectField:function(){$(this.element).removeClass("sfFieldEditMode").addClass("sfFieldEditable");
if(this.editor){$(this.editor.body).focusout();
}},linkManagerLinkSelected:function(e,HyperLinkManager){var linkAttributes=HyperLinkManager.getLinkAttributes();
var kendoUIDomAttrFunc=kendo.ui.editor.Dom.attr;
var sfDomAttrFunc=function(element,attributes){var extendedElement=kendoUIDomAttrFunc(element,attributes);
if(attributes.sfref&&!extendedElement.attributes.sfref){extendedElement.setAttribute("sfref",attributes.sfref);
...
},returnFocus:function(event,sender){this.editor.selectRange(this.editorRange);
},disableEditing:function(){this.element.removeAttr("contenteditable");
if(this.editorWindow){this.editorWindow.unbind("open");
...
if(container.find("[data-sf-ignore]").remove().length>0){this.value=container.html();
}this.value=$.trim(this.value);
}return this.value!==this.originalValue;
},onImageSelected:function(event,selectedImages){var selectedImage=selectedImages[0]||selectedImages;
...
},onImageEditorSelected:function(event,sender){var selectedImage=this.imageEditor.getSelectedImage();
this.updateImgTag(selectedImage);
sender.close();
},updateImgTag:function(imageItem){var oldHtmlSfref=this.imgTag.attr("sfref");
this.imgTag=this.imageEditor.formatImgTag(this.imgTag,imageItem);
if(this.insertImageTag){this.editor.exec("inserthtml",{value:this.imgTag[0].outerHTML});
}else{var formattedImgTag=this.imageEditor.formatImgTag(this.imgTag,imageItem);
