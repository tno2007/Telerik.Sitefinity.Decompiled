...
Telerik.Sitefinity.Modules.Libraries.Web.UI.Designers.CreateLibraryControl=function(element){this._cancelButton=null;
...
Telerik.Sitefinity.Modules.Libraries.Web.UI.Designers.CreateLibraryControl.prototype={initialize:function(){Telerik.Sitefinity.Modules.Libraries.Web.UI.Designers.CreateLibraryControl.callBaseMethod(this,"initialize");
if(this._blankLibraryDataItem){this._blankLibraryDataItem=this._deserializeBlankDataItem(this._blankLibraryDataItem);
}this.attachEventHandlers();
},createLibrary:function(name){this._createAlbumSimple(name);
},dispose:function(){this.detachEventHandlers();
},attachEventHandlers:function(){this.cancelClickDelegate=Function.createDelegate(this,this.cancelClickHandler);
...
},detachEventHandlers:function(){$removeHandler(this._cancelButton,"click",this.cancelClickDelegate);
...
if(!this._radioButtonsClickDelegate){if(this.get_noParent()){$removeHandler(this.get_noParent(),"click",this._radioButtonsClickDelegate);
...
}},rebind:function(providerName){this.get_parentLibrarySelector().set_providerName(providerName);
this.get_parentLibrarySelector().dataBind();
},createClickHandler:function(){if(!this.validate()){return;
...
},validate:function(){return this.get_createLibraryTxt().validate();
},cancelClickHandler:function(){this._handleCommandExecution("cancel");
this.set_libraryName("");
},radioButtonsClick:function(){jQuery(this.get_parentLibrarySelector().get_element()).toggle(this.get_selectParent().checked);
if((this.get_selectParent()).checked){this.get_parentLibrarySelector().set_providerName(this.get_provider());
this.get_parentLibrarySelector().dataBind();
}if(this.get_radWindow()){this.get_radWindow().AjaxDialog.resizeToContent();
}},reset:function(){this.get_noParent().checked=true;
...
},_raiseEvent:function(eventName,eventArgs){var handler=this.get_events().getHandler(eventName);
if(handler){handler(this,eventArgs);
}return eventArgs;
},_createEventArgs:function(commandName,args){return new Sys.CommandEventArgs(commandName,args,this);
},_handleCommandExecution:function(commandName,commandArgs,executeCustomCode){var eventArgs=this._raiseEvent("commandExecuting",this._createEventArgs(commandName,commandArgs));
if(eventArgs.get_cancel()){return;
}if(typeof(executeCustomCode)==="function"){executeCustomCode();
...
}if(dataObject.ParentId==null){if((this.get_selectParent()).checked&&this.get_parentLibrarySelector().get_selectedItem()){dataObject.ParentId=this.get_parentLibrarySelector().get_selectedItem().Id;
...
},_createAlbumSuccess:function(target,data,webRequest){target._raiseEvent("commandExecuted",target._createEventArgs("createLibrary",data.Item));
