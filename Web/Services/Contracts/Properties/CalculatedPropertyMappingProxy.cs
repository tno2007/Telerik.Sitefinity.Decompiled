// Decompiled with JetBrains decompiler
// Type: Telerik.Sitefinity.Web.Services.Contracts.CalculatedPropertyMappingProxy
// Assembly: Telerik.Sitefinity, Version=13.3.7600.0, Culture=neutral, PublicKeyToken=b28c218413bdf563
// MVID: 39C4B52A-B559-4D9C-97D9-CCCF73C3738E
// Assembly location: C:\Programs\Sitefinity\ProjectManager_13_3_7600\_EmptyProject\bin\Telerik.Sitefinity.dll

using System.Collections.Specialized;

namespace Telerik.Sitefinity.Web.Services.Contracts
{
  internal class CalculatedPropertyMappingProxy : 
    PropertyMappingProxy,
    ICalculatedPropertyMapping,
    IPropertyMapping
  {
    private string resolverType;
    private NameValueCollection parameters = new NameValueCollection();
    private bool selectedByDefault = true;

    public CalculatedPropertyMappingProxy()
    {
    }

    public CalculatedPropertyMappingProxy(ICalculatedPropertyMapping source)
      : base((IPropertyMapping) source)
    {
      this.resolverType = source.ResolverType;
      if (source.Parameters != null)
        this.parameters = new NameValueCollection(source.Parameters);
      this.selectedByDefault = source.SelectedByDefault;
    }

    public string ResolverType
    {
      get => this.resolverType;
      set => this.resolverType = value;
    }

    public NameValueCollection Parameters => this.parameters;

    public bool SelectedByDefault
    {
      get => this.selectedByDefault;
      set => this.selectedByDefault = value;
    }
  }
}
