...
Telerik.Sitefinity.Web.UI.Fields.HtmlField.prototype =
{
    ...
    dispose: function () {
        ...
        if (this._addDialogLoadHandlerDelegate) {
            delete this._addDialogLoadHandlerDelegate;
        }
    ...
    _handlePageLoad: function (sender, args) {
        ...
        if (this._editControl != null && this.get_displayMode() == Telerik.Sitefinity.Web.UI.Fields.FieldDisplayMode.Write) {
            ...
            Telerik.Web.UI.RadEditor.prototype._initContentAreaHtml = this._setRadEditorContentAreaHtml;
    ...
    _addDialogShowHandler: function (sender, args) {
        if (typeof dialogBase != "undefined") {
            var parentWindow = dialogBase.get_radWindow();
        ...
        if (args && args.isPageLoadEvent){
            if (typeof dialogBase != "undefined" && !parentWindow.IsMaximized()) {
               // Resize the content after the child dialog's content has been loaded
               sender.autoSize();
               sender.moveTo(50, 50);
           }
        }
    ...
    _addDialogLoadHandler: function (sender, args) {
        var that = this;
    ...
    onWindowClose: function (sender, eventArgs) {
        if (typeof dialogBase != "undefined") {
            if (dialogBase) {
    ...
    _setUnlinkCommand: function () {
        var $T = Telerik.Web.UI;
        var Editor = $T.Editor;
        var utils = Editor.Utils;

        Editor.UnlinkCommand = function (editor, settings, options) {
            settings = { tag: "a", altTags: [] };
        ...
        Editor.UnlinkCommand.prototype = {
            getState: function (wnd, editor, range) {
                ...
                var hasLink = false
                if (range) {
                    hasLink = utils.someInRange(range, function (node) {
                        var isTag = utils.isTag(node, tag);
            ...
            isSameTagFormat: function (node) {
                return utils.isTag(node, this.settings.tag);
            },
            shouldRemoveFormatting: function () {
                return true;
            },
        ...
        Editor.CommandList.Unlink = function (commandName, editor, oTool) {
            editor.setActive();
            editor.setFocus();
    ...
    get_value: function () {
        if (this._editControl != null && this.get_displayMode() == Telerik.Sitefinity.Web.UI.Fields.FieldDisplayMode.Write) {
            // In Html mode, content filters are not applied because of a bug in the RadEditor.
            // So we switch to design mode to apply them.
            if (this._editControl.get_mode() == Telerik.Web.UI.EditModes.Html) {
                this._editControl.set_mode(Telerik.Web.UI.EditModes.Design);
