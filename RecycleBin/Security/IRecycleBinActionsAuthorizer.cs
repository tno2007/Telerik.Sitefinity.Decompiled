// Decompiled with JetBrains decompiler
// Type: Telerik.Sitefinity.RecycleBin.Security.IRecycleBinActionsAuthorizer
// Assembly: Telerik.Sitefinity, Version=13.3.7600.0, Culture=neutral, PublicKeyToken=b28c218413bdf563
// MVID: 39C4B52A-B559-4D9C-97D9-CCCF73C3738E
// Assembly location: C:\Programs\Sitefinity\ProjectManager_13_3_7600\_EmptyProject\bin\Telerik.Sitefinity.dll

namespace Telerik.Sitefinity.RecycleBin.Security
{
  /// <summary>
  /// Defines common authorization methods for Recycle Bin operations.
  /// </summary>
  public interface IRecycleBinActionsAuthorizer
  {
    /// <summary>
    /// Ensures that there is a 'Delete' permission for the recyclable item when move data item to Recycle Bin.
    /// </summary>
    /// <param name="item">The recyclable item that is checked for permissions.</param>
    void EnsureMoveToRecycleBinPermissions(object item);

    /// <summary>
    /// Ensures that there is a 'Create' permission for the recyclable item when restore data item from Recycle Bin.
    /// </summary>
    /// <param name="item">The recyclable item that is checked for permissions.</param>
    void EnsureRestoreFromRecycleBinPermissions(object item);
  }
}
