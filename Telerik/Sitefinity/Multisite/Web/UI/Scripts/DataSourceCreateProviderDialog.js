...
Telerik.Sitefinity.Multisite.Web.UI.DataSourceCreateProviderDialog=function(element){Telerik.Sitefinity.Multisite.Web.UI.DataSourceCreateProviderDialog.initializeBase(this,[element]);
...
Telerik.Sitefinity.Multisite.Web.UI.DataSourceCreateProviderDialog.prototype={initialize:function(){Telerik.Sitefinity.Multisite.Web.UI.DataSourceCreateProviderDialog.callBaseMethod(this,"initialize");
...
},reset:function(){jQuery(this.get_providerName()).val("");
this._hideError();
},show:function(dataSource,dataSourceConfig){this.reset();
...
},_updateLabelsAndMessages:function(){var titleText=String.format(this.get_clientLabelManager().getLabel("MultisiteResources","CreateNewSourceFor"),this._siteDataSourceConfig.Title);
...
if(this.usersDataSourceMode){titleText=this.get_clientLabelManager().getLabel("MultisiteResources","CreateNewUserGroup");
...
},_doneButtonClickHandler:function(sender,args){var providerName=$(this.get_providerName()).val().trim();
if(providerName===""){this._showError(this.get_clientLabelManager().getLabel("MultisiteResources","EmptySourceError"));
return;
}if(providerName.length>255){this._showError(this.get_clientLabelManager().getLabel("MultisiteResources","SourceNameLengthMessage"));
return;
}if(this._validateIfProviderExist(providerName)){this.close({providerName:providerName});
}else{if(this.usersDataSourceMode){this._showError(this.get_clientLabelManager().getLabel("MultisiteResources","UserGroupAlreadyExistsError"));
}else{this._showError(this.get_clientLabelManager().getLabel("MultisiteResources","SourceAlreadyExistsError"));
}}},_validateIfProviderExist:function(providerName){var valid=true;
if(this._dataSource&&this._dataSource.data()&&this._dataSource.data().length){for(var i=0;
i<this._dataSource.data().length;
i++){if(providerName.toLowerCase()===this._dataSource.data()[i].Link.ProviderTitle.toLowerCase()){valid=false;
break;
}}}return valid;
...
},get_sourceInformationText:function(){return this._sourceInformationText;
},set_sourceInformationText:function(value){this._sourceInformationText=value;
},get_userGroupInformationText:function(){return this._userGroupInformationText;
},set_userGroupInformationText:function(value){this._userGroupInformationText=value;
},};
