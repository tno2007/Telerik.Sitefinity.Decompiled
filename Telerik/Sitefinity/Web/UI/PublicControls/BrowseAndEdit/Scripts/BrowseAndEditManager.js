...
function GetBrowseAndEditManager(){return Telerik.Sitefinity.BrowseAndEditManager;
}Telerik.Sitefinity.Web.UI.PublicControls.BrowseAndEdit.BrowseAndEditManager=function(element){Telerik.Sitefinity.Web.UI.PublicControls.BrowseAndEdit.BrowseAndEditManager.initializeBase(this,[element]);
this._toolbarIds=[];
this._toolbars=null;
...
Telerik.Sitefinity.Web.UI.PublicControls.BrowseAndEdit.BrowseAndEditManager.prototype={initialize:function(){Telerik.Sitefinity.Web.UI.PublicControls.BrowseAndEdit.BrowseAndEditManager.callBaseMethod(this,"initialize");
...
}if(this._toolbarCommandDelegate){this._unregisterToolbars();
...
if(this._dialogBeforeShowDelegate){if(dialogManager){dialogManager.remove_dialogBeforeShow(this._dialogBeforeShowDelegate);
}delete this._dialogBeforeShowDelegate;
}if(this._dialogClosedDelegate){if(dialogManager){dialogManager.remove_dialogClosed(this._dialogClosedDelegate);
...
},toggleBrowseAndEdit:function(display){if(!this._toolbars||this._toolbars.length==0){return;
}var toolbar,i,l;
if(typeof display==="undefined"){for(i=0,l=this._toolbars.length;
i<l;
i++){toolbar=this._toolbars[i];
if(typeof toolbar.get_alwaysVisible==="function"&&!toolbar.get_alwaysVisible()){display=!toolbar.get_visible();
break;
}}}if(typeof display!=="undefined"){for(i=0,l=this._toolbars.length;
i<l;
i++){toolbar=this._toolbars[i];
if(typeof toolbar.get_alwaysVisible==="function"&&!toolbar.get_alwaysVisible()){toolbar.set_visible(display);
}}}},_handlePageLoad:function(sender,args){this._registerToolbars();
...
},_toolbarCommandHandler:function(sender,args){this._checkPageForChanges({Sender:sender,Args:args});
},_executeToolbarCommand:function(sender,args){var dialogManager=GetDialogManager();
...
var dialog;
if(dialogName){dialog=dialogManager.getDialogByName(dialogName);
}if(dialog){this._toolbarArgs=args;
...
binder.get_provider=function(){return args.get_providerName();
};
return binder;
};
...
var settings={};
if(dialog._name=="PropertyEditor"){dialog.set_initialBehaviors(Telerik.Web.UI.WindowBehaviors.None);
dialog.set_width(425);
dialog.set_height(250);
dialog.set_modal(true);
dialog._cssClass="";
settings={maximize:false};
}else{dialog.set_initialBehaviors(Telerik.Web.UI.WindowBehaviors.Maximize);
...
if(dialogPrefix){var windowNameGenerator=function(name){return dialogPrefix+name;
};
dialogManager.set_windowNameGenerator(windowNameGenerator);
}}else{this._handleNondialogCommand(commandName,args);
...
},_dialogClosedHandler:function(sender,args){var dialogArgument=args.get_originalDialogEventArgument().get_argument();
if(dialogArgument!=null&&(dialogArgument==="rebind"||dialogArgument==="reload"||dialogArgument.DataItem)){this._reloadPage();
}},_messageWorkflow_Success:function(caller,result,request,context){if(context.url){this._loadPage(context.url);
}this._reloadPage();
},_messageWorkflow_Failure:function(sender,result){alert("Failed workflow operation");
...
}if(browseAndEditEnabled){this.toggleBrowseAndEdit(true);
}},_registerToolbars:function(){this._toolbars=[];
var toolbar;
for(var i=0,l=this._toolbarIds.length;
i<l;
i++){toolbar=$find(this._toolbarIds[i]);
if(toolbar){this._toolbars.push(toolbar);
toolbar.add_command(this._toolbarCommandDelegate);
}}},_unregisterToolbars:function(){if(this._toolbars&&this._toolbars.length>0){for(var i=0,l=this._toolbars.length;
i<l;
i++){this._toolbars[i].remove_command(this._toolbarCommandDelegate);
}}},_reloadPage:function(){var url=location.href;
if(location.hash){url=url.substr(0,url.indexOf("#"));
}location.href=url;
},_loadPage:function(url){location.href=url;
},_handleNondialogCommand:function(commandName,args){switch(commandName){case"toggleBrowseAndEdit":this.toggleBrowseAndEdit();
break;
case"delete":this._sendMessageWorkflow("Delete",args,{url:this._pageUrl});
break;
case"unpublish":this._sendMessageWorkflow("Unpublish",args,{url:this._pageUrl});
break;
}},_sendMessageWorkflow:function(workflowOperation,args,context){var clientManager=this.get_clientManager();
var urlParams=[];
...
},_checkPageForChanges:function(context){var clientManager=this.get_clientManager();
var urlParams=[];
...
},_calculateUrl:function(dialog,args){if(!args.get_urlParameters){return;
}var paramsArray=args.get_urlParameters();
if(paramsArray&&paramsArray.length>0){var url=new Sys.Uri(dialog.get_navigateUrl());
paramsLength=paramsArray.length;
for(var i=0;
i<paramsLength;
i++){var paramKeyValuePair=paramsArray[i];
if(("Key" in paramKeyValuePair)&&("Value" in paramKeyValuePair)){url.get_query()[paramKeyValuePair.Key]=paramKeyValuePair.Value;
}}dialog.set_navigateUrl(url.toString());
}},_dialogBeforeShowHandler:function(sender,args){var dialog=args.get_dialog();
this._calculateUrl(dialog,this._toolbarArgs);
},_checkPageSuccessHandler:function(context,args){switch(args.ItemState){case 0:this._executeToolbarCommand(context.Sender,context.Args);
break;
case 1:case 2:case 3:this.get_pageChangedWarningDialog().show_prompt(null,args.Message);
break;
}},_checkPageFailureHandler:function(sender,args){alert(args.Detail);
},get_clientManager:function(){if(this._clientManager==null){this._clientManager=new Telerik.Sitefinity.Data.ClientManager();
if(this._isMultilingual){this._clientManager.set_uiCulture(this._uiCulture);
