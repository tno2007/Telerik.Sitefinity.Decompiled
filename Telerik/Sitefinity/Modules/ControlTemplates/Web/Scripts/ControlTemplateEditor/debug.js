...
var controlTemplateEditor = null;

Telerik.Sitefinity.Modules.ControlTemplates.Web.UI.ControlTemplateEditor = function (element) {
    Telerik.Sitefinity.Modules.ControlTemplates.Web.UI.ControlTemplateEditor.initializeBase(this, [element]);
...
Telerik.Sitefinity.Modules.ControlTemplates.Web.UI.ControlTemplateEditor.prototype = {

    // ------------------------------------- Initialization -------------------------------------
    initialize: function () {
        Telerik.Sitefinity.Modules.ControlTemplates.Web.UI.ControlTemplateEditor.callBaseMethod(this, "initialize");

        controlTemplateEditor = this;
        ...
        if (this._controlTypesList) {
            this._valueChangedDelegate = Function.createDelegate(this, this._valueChangedHandler);
            $addHandler(this._controlTypesList.get_choiceElement(), "change", this._valueChangedDelegate);
        }

        if (this.get_itemTypesNamesSelector()) {
            jQuery(this.get_itemTypesNamesSelector().get_element()).hide();
        }

        if (this._commonPropertiesItemsList && this._otherPropertiesItemsList) {
            this._dialogClosedDelegate = Function.createDelegate(this, this._dialogClosedHandler);
            this._commonPropertiesItemsList.add_dialogClosed(this._dialogClosedDelegate);
            this._otherPropertiesItemsList.add_dialogClosed(this._dialogClosedDelegate);
        }

        if (this._expandCommonPropertiesButton) {
            this._toggleCommonPropertiesDelegate = Function.createDelegate(this, this._toggleCommonProperties);
            $addHandler(this._expandCommonPropertiesButton, "click", this._toggleCommonPropertiesDelegate);
        }

        if (this._expandOtherPropertiesButton) {
            this._toggleOtherPropertiesDelegate = Function.createDelegate(this, this._toggleOtherProperties);
            $addHandler(this._expandOtherPropertiesButton, "click", this._toggleOtherPropertiesDelegate);
        }

        this._restoreConfirmedDelegate = Function.createDelegate(this, this._restoreConfirmedHandler);
        this.get_restorePrompt().add_command(this._restoreConfirmedDelegate);

        this._restoreSuccessDelegate = Function.createDelegate(this, this._restoreSuccessHandler);
        this._restoreErrorDelegate = Function.createDelegate(this, this._restoreErrorHandler);
        this._itemTypeFullNameSelectionChangedDelegate = Function.createDelegate(this, this._itemTypeFullNameSelectionChangedHandler);
        this._getItemTypesSuccessDelegate = Function.createDelegate(this, this._getItemTypesSuccessHandler);
        this._getItemTypesFailureDelegate = Function.createDelegate(this, this._getItemTypesFailureHandler);
    },
    ...
    dispose: function () {
        ...
        Telerik.Sitefinity.Modules.ControlTemplates.Web.UI.ControlTemplateEditor.callBaseMethod(this, "dispose");
    },
    ...
    createEditor: function (commandName, dataItem, self, dialog, params, key) {
        ...
        if (params.ItemTypeFullName) {
            this._itemTypeFullName = params.ItemTypeFullName;
        }

        if (params.ControlType) {

            if (params.ModuleName != null) {
                this._controlType = params.ControlType + "-" + params.ModuleName;
                this._moduleName = params.ModuleName;
            }
            else {
                this._controlType = params.ControlType;
            }
            if (this.get_controlTypeSelector()) {
                jQuery(this.get_controlTypeSelector().get_element()).hide();
                this._setAppliedToTemplate();
            }
        }

        if (params.Condition && this._templateCondition == null) {
            this._templateCondition = params.Condition;
        }

        if (params.ItemTypesServiceUrl && params.ItemTypeFullNameField && params.ItemTypeTitleField) {
            this._itemTypeFullNameField = params.ItemTypeFullNameField;
            this._itemTypeTitleField = params.ItemTypeTitleField;
            this._configureItemTypeNamesSelector(params.ItemTypesServiceUrl);
        }
        ...
        if (dataItem) {
            ...
            if (this._blankDataItem != null) {
                this._blankDataItem.ControlType = this._controlType;
                this._blankDataItem.Condition = this._templateCondition;
                this._blankDataItem.DataType = params.DataType;
                var newItemContext = new Object();
                newItemContext["Item"] = this._blankDataItem;
                this._binder.BindItem(newItemContext);
            }

            if (this.get_moduleTitle())
                this._setModuleTitle();
            if (this.get_commonPropertiesItemsList())
    ...
    saveChanges: function () {
        this._isDirty = true;
        this.get_templateDataField().set_value(this.get_CodeMirror().getValue());
        var item = this._binder.get_dataItem();

        if (this._getModuleTitle() !== undefined)
            item["AreaName"] = this._getModuleTitle();

        item["FriendlyControlName"] = this._getAppliedToTemplateName();

        // The allProducts check prevents the template editor from setting the condition of the Product Details template to the
        // value of the "Insert fields of..." dropdown, so that the template will not be constrained to that type of product.
        if (item["Condition"] == null && this._templateCondition !== 'allProducts') {
            item["Condition"] = this._getItemTypeFullName();
        }

        this._binder.set_dataItem(item);
    ...
    getInsertSectionContainer: function(){
        return jQuery(this.get_otherPropertiesContainer()).parent();
    },
    ...
    _commandHandler: function (sender, args) {
        switch (args.get_commandName()) {
            ...
            case this._cancelCommandName:
                this.getInsertSectionContainer().hide();
                this.close();
                break;
            case this._previewCommandName:
                alert("In process of implementation.");
                break;
            case "openHistoryGrid":
                this.openHistoryGrid()
                break;
            case this._restoreCommandName:
                this.restoreTemplate();
                break;
        }
    },

    openHistoryGrid: function () {

        this._itemsBase.executeItemCommand("historygrid", this._binder.get_dataItem(), this._key, null);
        //var dialogManager = window.top.GetDialogManager();
        //var dialog = dialogManager.getDialogByName("historygrid");
      
        //var context = { commandName: "historygrid", dataItem: this._binder.get_dataItem(), itemsList: null, dialog: dialog, params: this._params, key: this._key };
        //dialogManager.openDialog("historygrid", this, context)
    },
    ...
    _dataBindSuccess: function (sender, result) {
        ...
        this.Caller._dataBindPropertiesItemsList(this.Caller._commonPropertiesItemsList);
...
Telerik.Sitefinity.Modules.ControlTemplates.Web.UI.ControlTemplateEditor.registerClass("Telerik.Sitefinity.Modules.ControlTemplates.Web.UI.ControlTemplateEditor", Telerik.Sitefinity.Web.UI.AjaxDialogBase);
