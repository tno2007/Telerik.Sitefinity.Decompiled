...
// Type: Telerik.Sitefinity.Web.Services.Contracts.Operations.Pages.PropertyEditor.Adaptors.IPropertiesAdaptor
...
{
  internal interface IPropertiesAdaptor
  {
    ControlMetadata AdaptControlMetadata(IAdaptControlArgs args);

    IList<PropertyValueContainer> AdaptValuesForSerialization(
      IEnumerable<WcfControlProperty> wcfProperties,
      IAdaptValuesContext context);

    IEnumerable<WcfControlProperty> AdaptValuesForPersistence(
      IEnumerable<PropertyValueContainer> properties,
      IEnumerable<WcfControlProperty> sourceProperties);

    bool CanAdaptComponent(ControlData component);
