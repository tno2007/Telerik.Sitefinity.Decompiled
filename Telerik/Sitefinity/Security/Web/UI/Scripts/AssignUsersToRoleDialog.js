...
Telerik.Sitefinity.Web.UI.AssignUsersToRoleDialog=function(id){Telerik.Sitefinity.Web.UI.AssignUsersToRoleDialog.initializeBase(this,[id]);
...
Telerik.Sitefinity.Web.UI.AssignUsersToRoleDialog.prototype={initialize:function(){Telerik.Sitefinity.Web.UI.AssignUsersToRoleDialog.callBaseMethod(this,"initialize");
...
},dispose:function(){Telerik.Sitefinity.Web.UI.AssignUsersToRoleDialog.callBaseMethod(this,"dispose");
...
this._saveButton=null;
this._cancelButton=null;
if(this._pickRolesManager!=null){this._pickRolesManager.dispose();
this._pickRolesManager=null;
}if(this._saveManager!=null){this._saveManager.dispose();
this._saveManager=null;
}if(this._countManager!=null){this._countManager.dispose();
...
},_pageLoad:function(sender,args){this._tabSwithcer=$find(this._tabSwithcer);
...
},_initCountManager:function(){if(!this._countManager){this._countManager=new Telerik.Sitefinity.Data.ClientManager();
}this._updateUsersProviderName();
},_initPickRolesManager:function(){if(!this._pickRolesManager){this._pickRolesManager=new Telerik.Sitefinity.Data.ClientManager();
}this._updateUsersProviderName();
},_initSaveManager:function(){if(!this._saveManager){this._saveManager=new Telerik.Sitefinity.Data.ClientManager();
}},_initAllUsersBinder:function(){this._updateUsersProviderName();
...
this._allUsersBinder.set_urlParams({forAllProviders:forAllProviders,roleProvider:this._rolesProviderName,roleId:this._roleId});
},_initUsersInRoleBinder:function(){this._updateUsersProviderName();
...
this._inRoleBinder.set_urlParams({userProvider:this._usersProviderName,forAllUserProviders:forAllUserProviders});
var self=this;
this._inRoleBinder.get_manager()._getItemCollection=function(successDelegate,binder,dataKeys,context){if(binder.get_provider()!=null){this.get_urlParams()["provider"]=binder.get_provider();
...
}if(binder.get_filterExpression()!=null){this.get_urlParams()["filter"]=binder.get_filterExpression();
...
},dataBind:function(){if(typeof this._roleId=="string"){this._titleLabel.innerHTML=String.format(this._title,this._roleName);
var mode=this.get_mode();
this._messageControl.hide();
this._setIsServiceSelection(true);
if(mode=="allUsers"){this.binderAllUsers();
}else{if(mode=="usersInRole"){this.bindUsersInRole();
}else{this._raiseError(Object.getTypeName(this)+".dataBind(): Invalid mode : "+mode);
}}}else{this._raiseError("You must call set_roleId before calling dataBind");
}},binderAllUsers:function(){this._initAllUsersBinder();
this._allUsersBinder.DataBind();
},bindUsersInRole:function(){this._initUsersInRoleBinder();
this._inRoleBinder.DataBind();
},pickSelectedUsersFromRange:function(usersToPickFrom){this._initPickRolesManager();
var iter=usersToPickFrom.length;
var idsToPickFrom=[];
while(iter--){idsToPickFrom.push(usersToPickFrom[iter].UserID);
}this._pickRolesManager.InvokePut(this._rolesServiceUrl,{provider:this._rolesProviderName!=null?this._rolesProviderName:""},["pick",this._roleId],idsToPickFrom,this._pickedRolesReceivedCallback,this._raiseError,this,false,"",usersToPickFrom);
},getAllUsersCount:function(){},_isUserStored:function(id,providerName){var key=this._getChangeKey(id,providerName);
return this._change.hasOwnProperty(key);
},_getStoredUser:function(id,providerName){var key=this._getChangeKey(id,providerName);
if(this._change.hasOwnProperty(key)){return this._change[key];
}else{return null;
}},_getAllStoredUsers:function(){var storedUsers=[];
for(var key in this._change){if(this._change.hasOwnProperty(key)){storedUsers.push(this._change[key]);
}}return storedUsers;
},_storeUserToRemove:function(id,providerName){this._storeUserChange(id,providerName,true);
},_storeUserToAdd:function(id,providerName){this._storeUserChange(id,providerName,false);
},_storeUserChange:function(id,providerName,remove){var key=this._getChangeKey(id,providerName);
this._changeSelectedUsersCount(remove?-1:1);
if(!this._change.hasOwnProperty(key)){this._change[key]={UserId:id,ProviderName:providerName,Remove:remove};
}else{this._change[key].Remove=remove;
}},_getChangeKey:function(id,providerName){return providerName+"/"+id;
},_intersectSelectedIdsWithChange:function(visuallySelectedUsers){var intersected=[];
var iter=visuallySelectedUsers.length;
while(iter--){var user=visuallySelectedUsers[iter];
var stored=this._getStoredUser(user.UserID,user.ProviderName);
if(stored==null||!stored.Remove){intersected.push(user.UserID);
}}var storedUsers=this._getAllStoredUsers();
var iter=storedUsers.length;
while(iter--){var stored=storedUsers[iter];
var existingIter=intersected.length;
var exists=false;
while(existingIter--){if(intersected[existingIter]==stored.UserId){exists=true;
break;
}}if(exists){continue;
}if(stored.Remove==false&&(stored.ProviderName==this._usersProviderName||this._usersProviderName==null)){intersected.push(stored.UserId);
}}return intersected;
},_saveChanges:function(sender,args){var storedUsers=this._getAllStoredUsers();
if(storedUsers.length>0){this._initSaveManager();
this._saveManager.InvokePut(this._rolesServiceUrl,{provider:this._rolesProviderName},["SaveRoleUser",this._roleId],storedUsers,this._changesSavedCallback,this._raiseError,this);
}else{this._change=new Object();
dialogBase.close();
}},_changesSavedCallback:function(caller,data,request){caller._changesSaved.apply(caller,[]);
},_changesSaved:function(){this._change=new Object();
dialogBase.closeAndRebind();
},_cancelChanges:function(sender,args){this._change=new Object();
dialogBase.close();
},_allUsersCountReceivedCallback:function(caller,data,request){caller._allUsersCountReceived.apply(caller,[data]);
},_allUsersCountReceived:function(data){this._setSelectedUsersCount(parseInt(data));
},_doSearch:function(sender,args){var searchFilter=args.get_query();
this.set_filterExpression(searchFilter);
this.dataBind();
},_rowSelecting:function(sender,args){},_rowDeselecting:function(sender,args){},_rowSelected:function(sender,args){if(!this._isServiceSelection()){var gridItem=args.get_gridDataItem();
var dataItem=gridItem.get_dataItem();
this._storeUserToAdd(dataItem.UserID,dataItem.ProviderName);
}},_rowDeselected:function(sender,args){if(!this._isServiceSelection()){var gridItem=args.get_gridDataItem();
var dataItem=gridItem.get_dataItem();
this._storeUserToRemove(dataItem.UserID,dataItem.ProviderName);
}},_providersListChange:function(sender,args){this._updateUsersProviderName();
this.dataBind();
},_updateUsersProviderName:function(){var idx=this._providersList.selectedIndex;
if(idx==0){this._usersProviderName=null;
}else{this._usersProviderName=this._providersList.options[idx].value;
}return this._usersProviderName;
},_tabSelected:function(sender,args){this.dataBind();
},_allUsersDataBound:function(sender,args){var collectionContext=args.get_dataItem();
...
if(collectionContext.Items.length>0){this.pickSelectedUsersFromRange(collectionContext.Items);
}},_inRoleDataBound:function(sender,args){var collectionContext=args.get_dataItem();
...
while(gridItemIter--){var gridItem=gridItems[gridItemIter];
var dataItem;
var desiredID;
try{dataItem=gridItem.get_dataItem();
desiredID=dataItem.UserID;
}catch(error){continue;
}var userIter=usersInRolePerPage.length;
while(userIter--){if(desiredID==usersInRolePerPage[userIter].UserID){gridItem.set_selected(true);
}}}this._setIsServiceSelection(false);
},_pickedRolesReceivedCallback:function(caller,data,request){caller._pickedRolesReveived.apply(caller,[data,request.get_userContext().Context]);
},_pickedRolesReveived:function(data,usersInPage){var ids=data.Items;
var selectedUsers=[];
var idIter=ids.length;
while(idIter--){var neededID=ids[idIter];
var userIter=usersInPage.length;
while(userIter--){var user=usersInPage[userIter];
if(user.UserID==neededID){selectedUsers.push(user);
break;
...
while(gridItemIter--){var gridItem=gridItems[gridItemIter];
var desiredID;
try{desiredID=gridItem.get_dataItem().UserID;
}catch(error){continue;
}var idIter=ids.length;
while(idIter--){if(desiredID==ids[idIter]){gridItem.set_selected(true);
}}}this._setIsServiceSelection(false);
},_binderError:function(sender,args){this._raiseError(args.get_error());
...
},_raiseError:function(error){error=error?error:"";
error=error.Detail?error.Detail:error;
var self=this.Caller?this.Caller:this;
var handler=self.get_events().getHandler("error");
if(handler!=null){var args=self._getErrorArgs(error);
handler(self,args);
}else{self._messageControl.showNegativeMessage(error);
}},_getErrorArgs:function(message){var args=new Sys.EventArgs();
...
if(!fieldName){fieldName="_"+propertyName.charAt(0)+propertyName.slice(1);
}if(!fieldName||!this.hasOwnProperty(fieldName)){return;
}var currentValue=this[fieldName];
if(currentValue!=value){this[fieldName]=value;
this.raisePropertyChanged(propertyName);
}},_setAllUsersTitle:function(count){if(this._showAllUsersCount){count=typeof count==typeof 1?"("+count+")":"";
}else{count="";
}this._allUsersLabel.innerHTML=String.format(this._allUsersTitle,count);
},_setSelectedUsersCount:function(count){this._selectedUsersCountInternal=count;
this._setSelectedUsersTitle(this._selectedUsersCountInternal);
},_changeSelectedUsersCount:function(changeBy){this._selectedUsersCountInternal+=changeBy;
this._setSelectedUsersTitle(this._selectedUsersCountInternal);
},_setSelectedUsersTitle:function(count){count=typeof count==typeof 1?"("+count+")":"";
...
},_setIsServiceSelection:function(enable){this._serviceSelectionEnabled=enable;
},_isServiceSelection:function(){return this._serviceSelectionEnabled;
},get_mode:function(){if(this._tabSwithcer.get_selectedTab().get_index()==0){return"allUsers";
}else{return"usersInRole";
