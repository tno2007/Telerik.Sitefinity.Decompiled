...
Telerik.Sitefinity.Modules.Security.Permissions.Web.UI.Views.GlobalPermissionsView=function(element){this._permissionsControlSelectorID=null;
...
Telerik.Sitefinity.Modules.Security.Permissions.Web.UI.Views.GlobalPermissionsView.prototype={initialize:function(){Telerik.Sitefinity.Modules.Security.Permissions.Web.UI.Views.GlobalPermissionsView.callBaseMethod(this,"initialize");
Sys.Application.add_load(Function.createDelegate(this,this.onload));
},dispose:function(){Telerik.Sitefinity.Modules.Security.Permissions.Web.UI.Views.GlobalPermissionsView.callBaseMethod(this,"dispose");
},onload:function(){var tabStrip=$find(this._permissionsControlSelectorID);
...
},bindSelectedModule:function(module){if(module!=null){var globalItemsActionPermissionsList=$find(this._globalItemsActionPermissionsListID);
if(module.IsDynamicModule){globalItemsActionPermissionsList.set_applyDynamicModulePermissions(true);
globalItemsActionPermissionsList.set_permissionSetName(this._dynamicModulePermissionSet);
}else{globalItemsActionPermissionsList.set_applyDynamicModulePermissions(false);
...
}},_module_Selected:function(sender,args){this.bindSelectedModule(args);
},_module_SelectorLoaded:function(){var moduleSelector=$find(this._moduleSelectorID);
this.bindSelectedModule(moduleSelector.get_selectedModule());
},_user_Selected:function(sender,selectedItem){if(typeof(selectedItem.UserID)!="undefined"){var userId=selectedItem.UserID;
var userTitle=selectedItem.FirstName+" "+selectedItem.LastName;
if(((selectedItem.FirstName=="")&&(selectedItem.LastNam==""))||((selectedItem.FirstName==null)&&(selectedItem.LastNam==null))){userTitle=selectedItem.Email;
}userName=userTitle;
this._bindByUser(userId,userName);
}},_user_SelectorReady:function(sender,args){sender.setFirstRowActive();
},_role_Selected:function(sender,selectedItem){if(typeof(selectedItem.Id)!="undefined"){var roleId=selectedItem.Id;
var roleName=selectedItem.Name;
this._bindByRole(roleId,roleName);
}},_role_SelectorReady:function(sender,args){sender.setFirstRowActive();
},_tabStrip_TabSelected:function(){var tabStrip=$find(this._permissionsControlSelectorID);
...
if((rtSection!=null)&&(rtSection.get_selected())&&(globalItemsActionPermissionsList!=null)){var moduleSelector=$find(this._moduleSelectorID);
this.bindSelectedModule(moduleSelector.get_selectedModule());
}else{if((rtUser!=null)&&(rtUser.get_selected())&&(globalUserPermissionsList!=null)){globalUserPermissionsList.dataBind();
}else{if((rtRole!=null)&&(rtRole.get_selected())&&(globalUserPermissionsListRoles!=null)){globalUserPermissionsListRoles.dataBind();
}}}},_bindByUser:function(userId,userName){if(userId!=null){var globalUserPermissionsList=$find(this._globalUserPermissionsListID);
globalUserPermissionsList.set_principalID(userId);
if(userName!=null){globalUserPermissionsList.set_principalName(userName);
globalUserPermissionsList.set_showPrincipalName(true);
}globalUserPermissionsList.dataBind();
}},_bindByRole:function(roleId,roleName){if(roleId!=null){var globalUserPermissionsListRoles=$find(this._globalUserPermissionsListRolesID);
globalUserPermissionsListRoles.set_principalID(roleId);
if(roleName!=null){globalUserPermissionsListRoles.set_principalName(roleName);
