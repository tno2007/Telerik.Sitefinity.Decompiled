...
Telerik.Sitefinity.Web.UI.Fields.ListPipeSettingsField = function (element) {
    this._existingPipesBinder = null;
    //holds the displayed pipes
    this._value = [];
...
Telerik.Sitefinity.Web.UI.Fields.ListPipeSettingsField.prototype =
{
    initialize: function () {
        Telerik.Sitefinity.Web.UI.Fields.ListPipeSettingsField.callBaseMethod(this, "initialize");

        if (this._itemCommandHandlerDelegate == null)
            this._itemCommandHandlerDelegate = Function.createDelegate(this, this._itemCommandHandler);
        if (this._dataBoundHandlerDelegate == null)
            this._dataBoundHandlerDelegate = Function.createDelegate(this, this._dataBoundHandler);
        if (this._windowLoadHandlerDelegate == null)
            this._windowLoadHandlerDelegate = Function.createDelegate(this, this._windowLoadHandler);
        if (this._dialogDoneHandlerDelegate == null)
            this._dialogDoneHandlerDelegate = Function.createDelegate(this, this._dialogDoneHandler);
        if (this._addSettingsHandlerDelegate == null)
            this._addSettingsHandlerDelegate = Function.createDelegate(this, this._addSettingsHandler);
        if (this._itemDataBoundHandlerDelegate == null)
            this._itemDataBoundHandlerDelegate = Function.createDelegate(this, this._itemDataBoundHandler);
        if (this._loadHandlerDelegate == null)
        ...
        if (this._showDefaultPipes) {
            this._value = this._defaultPipes;
    ...
    dispose: function () {
        Telerik.Sitefinity.Web.UI.Fields.ListPipeSettingsField.callBaseMethod(this, "dispose");

        $removeHandler(this._addSettingsButton, "click", this._addSettingsHandlerDelegate);
        if (this._existingPipesBinder) {
            this._existingPipesBinder.remove_onItemCommand(this._itemCommandHandlerDelegate);
        ...
        if (this._itemDataBoundHandlerDelegate)
            delete this._itemDataBoundHandlerDelegate;
        if (this._addSettingsHandlerDelegate)
            delete this._addSettingsHandlerDelegate;
        if (this._loadHandlerDelegate)
            delete this._loadHandlerDelegate;
        delete this._getLatestPublishingPoitDefinitionDelegate;

        if (this._pagePickerClosedDelegate)
            delete this._pagePickerClosedDelegate;
    },
    _loadHandler: function () {
        if (typeof GetDialogManager == "function") {
            GetDialogManager().blacklistWindowManager(this._windowManager);
        }
    ...
    set_fieldChangeNotifier: function (notifier) {
        this._fieldChangeNotifier = notifier;
    },

    // Gets the reference to the client binder that displays the existing pipes.
    get_existingPipesBinder: function () {
        return this._existingPipesBinder;
    },
    // Sets the reference to the client binder that displays the existing pipes.
    set_existingPipesBinder: function (value) {
        this._existingPipesBinder = value;
    },
    ...
    get_disableRemoving: function () {
        return this._disableRemoving;
    },
    set_disableRemoving: function (value) {
        this._disableRemoving = value;
    },
    get_changePipeText: function () {
        return this._changePipeText;
    },
    set_changePipeText: function (value) {
        this._changePipeText = value;
    },
    get_disableActivation: function () {
        return this._disableActivation;
    },
    set_disableActivation: function (value) {
        this._disableActivation = value;
    },
    get_showDefaultPipes: function () {
        return this._showDefaultPipes;
    },
    set_showDefaultPipes: function (value) {
        this._showDefaultPipes = value;
    },

    get_showContentLocation: function () {
        return this._showContentLocation;
    },
    set_showContentLocation: function (value) {
        this._showContentLocation = value;
    },
    ...
    set_value: function (value) {
        this._value = value;
        if (!value && this._showDefaultPipes) {
            if (this._existingPipesBinder) {
                this._value = this._defaultPipes;
            }
        }
        else {
            this._value = jQuery.extend(true, [], value);
        }
    ...
    get_defaultPipes: function () {
        return this._defaultPipes;
    },
    set_defaultPipes: function (value) {
        this._defaultPipes = value;
    },
    ///the default pipe settings added on clicking the add button
    get_defaultCreatePipe: function () {
        return this._defaultCreatePipe;
    },
    set_defaultCreatePipe: function (value) {
        this._defaultCreatePipe = value;
    },
    // gets or sets a reference to the add settings button
    get_addSettingsButton: function () {
        return this._addSettingsButton;
    },
    set_addSettingsButton: function (value) {
        this._addSettingsButton = value;
    },

    validate: function () {
        //if (this._showContentLocation) {
        //    for (var pipeIndex in this._value) {
        //        var pipe = this._value[pipeIndex];
        //        if (pipe.PipeName && this._pipeNeedsPageId(pipe.PipeName) && pipe.IsActive) {
        //            if (!pipe.ContentLocationPageID) {
        //                detailFormView.get_messageControl().showNegativeMessage("Please, specify default page for " + pipe.ContentName)
        //                return false;
        //            }
        //        }
        //    }
    ...
    _getDefaultSettings: function (pipeName, inbound) {
        //RegisteredInboundPipeSettings
        //RegisteredOutboundPipeSettings
        var registered;
        if (inbound) {
            registered = this._fieldChangeNotifier.get_fieldValue("RegisteredInboundPipeSettings");
        }
        else {
            registered = this._fieldChangeNotifier.get_fieldValue("RegisteredOutboundPipeSettings");
        }
        var iter = registered.length;
        while (iter--) {
            var pipe = registered[iter];
    ...
    _itemDataBoundHandler: function (sender, args) {
        if (this._disableRemoving) {
            var btn = args.FindControl("removeButton");
            jQuery(btn).hide();
        }
        if (this._changePipeText) {
            var elem = args.FindControl("changeButtonLabel");
            elem.innerHTML = this._changePipeText;
        }
        if (this._disableActivation) {
            var chck = args.FindControl("activateCheckbox");
            jQuery(chck).hide();
        }
        ...
        if (!this._showContentLocation) {
            if (pipe.ContentLocationPageID == null) {
                jQuery(loc).hide();
            }
        }
        else {
            if (!this._pipeNeedsPageId(args.get_dataItem().PipeName)) {
                jQuery(loc).hide();
                jQuery(uidesc).hide();
            }
            else {
    ...
    _itemCommandHandler: function (sender, args) {
        switch (args.get_commandName()) {
            case "editSettings":
                this._onSettingsChanged(sender, args);
                break;
            case "removeSettings":
                this._onSettingsDelete(sender, args);
                break;
            case "activate":
                var input = args.FindControl("pipeIsActive");
            ...
            case "removeLocation":
                this._removeContentLocation(args);
        }
    },
    _onSettingsChanged: function (sender, args) {
        this._currentPipeSettings = args.get_dataItem();
        this._currentPipeSettingsKey = args.get_itemIndex();
        var pipeName = args.get_dataItem().PipeName;
        this._openSettingsDialog(pipeName);
    },
    _onSettingsDelete: function (sender, args) {
        var elem = args.get_itemElement();
    ...
    _windowLoadHandler: function (sender, args) {
        ...
        if (typeof frameHandle.showSettings == "function") {
            var serializedData = null;
            var data = null;
            if (this._currentPipeSettings.Settings) {
                serializedData = this._currentPipeSettings.Settings;
                data = Sys.Serialization.JavaScriptSerializer.deserialize(serializedData);
            }
            var additionalData = null;
            if (this._currentPipeSettings.AdditionalSettings) {
                serializedData = this._currentPipeSettings.AdditionalSettings;
            ...
            if (settingsData.pipe[this._publishingPointNameFieldName] == null) {
                settingsData.pipe[this._publishingPointNameFieldName] = this._fieldChangeNotifier.get_fieldValue(this._publishingPointNameFieldName);
            }
            frameHandle.showSettings(settingsData, this._dialogDoneHandlerDelegate, this._currentPipeSettingsKey);
        }
    },
    _getLatestPublishingPoitDefinition: function () {
        var uiData = this._fieldChangeNotifier.get_fieldValuesFromUI(this._publishingPointDefinitionFieldName);
        var definition;
        if (typeof uiData != "undefined" && uiData.length > 0) {
            definition = uiData[0];
        }
        else {
            definition = this._fieldChangeNotifier.get_fieldValue(this._publishingPointDefinitionFieldName);
        }
        return definition;
    },
    _dialogDoneHandler: function (settingsData, settingsContext) {

        //settingsData = Sys.Serialization.JavaScriptSerializer.deserialize(settingsData);
        settingsData = Telerik.Sitefinity.JSON.parse(settingsData);
        //settingsData.pipe.MappingSettings = Telerik.Sitefinity.Publishing.Web.UI.Mapping.fixMappingSettings(settingsData.pipe.MappingSettings);
        //settingsData.pipe.Definition = Telerik.Sitefinity.fixArray(settingsData.pipe.Definition);
    ...
    _addSettingsHandler: function (e) {
        var dataItem = jQuery.extend(true, {}, this._defaultCreatePipe);
    ...
    _openSettingsDialog: function (pipeName) {
        var mode = this._outBoundPipesMode ? "Outbound" : "Inbound";
    ...
    _pagePickerOpenedHandler: function (sender, args) {
        if (this._currentArgs) {
            var currentPipe = this._currentArgs.get_dataItem();
            var currentPage = currentPipe.ContentLocationPageID;
            if (currentPage) {
                sender.get_pageSelector().set_selectedItemId(currentPage);
                sender.get_pageSelector().dataBind();
            }
        }
    },

    _pagePickerClosedHandler: function (sender, args) {
        if (this._currentArgs) {
            if (sender.get_value()) {
                if (sender._selectedPage) {
                    var currentPipe = this._currentArgs.get_dataItem();
                    ...
                    this._removeContentLocation(this._currentArgs);
                }
            }
        }
    },

    _removeContentLocation: function(args){
        var currentPipe = args.get_dataItem();
    ...
    _pipeNeedsPageId: function (pipeName) {
        return pipeName != "PagePipe" && pipeName != "DocumentSearchInboundPipe";
    }
};
