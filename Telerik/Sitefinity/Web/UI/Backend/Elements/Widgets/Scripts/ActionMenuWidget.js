...
Telerik.Sitefinity.Web.UI.Backend.Elements.Widgets.ActionMenuWidget=function(element){Telerik.Sitefinity.Web.UI.Backend.Elements.Widgets.ActionMenuWidget.initializeBase(this,[element]);
this._menu=null;
...
Telerik.Sitefinity.Web.UI.Backend.Elements.Widgets.ActionMenuWidget.prototype={initialize:function(){Telerik.Sitefinity.Web.UI.Backend.Elements.Widgets.ActionMenuWidget.callBaseMethod(this,"initialize");
...
}},dispose:function(){Telerik.Sitefinity.Web.UI.Backend.Elements.Widgets.ActionMenuWidget.callBaseMethod(this,"dispose");
if(this._commandDelegate){if(this._menu){this._menu.remove_itemClicked(this._commandDelegate);
}delete this._commandDelegate;
}},_commandHandler:function(sender,args){var commandName=args.get_item().get_value();
if(commandName!==null){var commandEventArgs=new Telerik.Sitefinity.UI.CommandEventArgs(commandName,null);
setTimeout(function(){sender.close();
},10);
var h=this.get_events().getHandler("command");
if(h){h(this,commandEventArgs);
}}},_loadHandler:function(){this._menu=$find(this._menuClientId);
...
},hideWidgetByName:function(name){var item=this._menu.findItemByValue(name);
if(item){item.hide();
$(item.get_element()).addClass("sfDisplayNoneImportant");
}else{throw Error.argument("name","No item found with the specified value.");
}},showWidgetByName:function(name){var item=this._menu.findItemByValue(name);
if(item){item.show();
$(item.get_element()).removeClass("sfDisplayNoneImportant");
}else{throw Error.argument("name","No item found with the specified value.");
}},hideEmptySections:function(){var allItems=this._menu.get_allItems();
var lastSeparator=null;
var hasVisible=false;
for(var i=0,length=allItems.length;
i<length;
i++){var item=allItems[i];
if(item.get_isSeparator()){if(!hasVisible){lastSeparator.hide();
}lastSeparator=item;
hasVisible=false;
}else{if(!hasVisible&&item.get_visible()){hasVisible=true;
}}}if(!hasVisible&&lastSeparator){lastSeparator.hide();
