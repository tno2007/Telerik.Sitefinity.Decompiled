...
// Provide a stub for IE when not debugging.
window.console = window.console || {};
if (typeof (window.console.log) === "undefined") {
    window.console.log = window.alert;
}

Telerik.Sitefinity.Web.UI.EditImagePropertiesDialog = function (element) {
    this._imageData;
    this._serviceBase;
    this._itemType;
    ...
    // delegate used the override the standard functionality
    this._updateImagePropertiesDelegate = null;

    Telerik.Sitefinity.Web.UI.EditImagePropertiesDialog.initializeBase(this, [element]);
}

Telerik.Sitefinity.Web.UI.EditImagePropertiesDialog.prototype = {

    /* ************************* set up and tear down ************************* */

    initialize: function () {
        Telerik.Sitefinity.Web.UI.EditImagePropertiesDialog.callBaseMethod(this, 'initialize');
        ...
        if (this._bodyCssClass) {
            jQuery("body").addClass(_bodyCssClass);
        }
    ...
    dispose: function () {
        ...
        if (this._librarySelectorField) {
            delete this._librarySelectorField;
        }
    ...
    setImageProperties: function (image) {
        this._displayFileName(image.FileName);
    ...
    _closeDialogCancel: function () {
        if (this._imageData) {
            this._unlockImage(this._imageData.Item.Id);
        }
        this.close();
    },

    _closeDialogSave: function (sender, args) {
        if (this._updateImagePropertiesDelegate) {
            this._updateImagePropertiesDelegate(null);
        }

        if (this._imageData) {
            this._saveImageData(this._imageData);
        }
    ...
    _getImageData: function (image) {
        var urlFormat = this.get_serviceBase() + "/parent/{0}/{1}/?itemType={2}&parentItemType={3}&checkOut=true";
    ...
    _saveImageData: function (imageData) {
        var urlFormat = this.get_serviceBase() + "/parent/{0}/{1}/?itemType={2}&parentItemType={3}&workflowOperation=Publish";
        var itemType = this.get_itemType();
        var parentItemType = this.get_parentItemType();
        ...
        var succeeded = function (sender, result) {
            unlockImage(idToUnlock);
        };
        var failed = function (error, caller) {
            unlockImage(idToUnlock);
            console.log(error);
        };
        ...
        if (this.get_librarySelectorField().control.isChanged()) {
            var albumId = this.get_librarySelectorField().control.getSelectedParentId();
    ...
    _unlockImage: function (idToUnlock) {
        var urlFormat = this.get_serviceBase() + "/temp/{0}/";
    ...
    _displayFileName: function (value) {
        this.get_imageOverviewPanel().find('.sfEditImagePropsFileName').html(value);
    },

    _displayFileSize: function (value) {
        this.get_imageOverviewPanel().find('.sfEditImagePropsFileSize').html(value);
    },

    _displayUploadedDate: function (value) {
        this.get_imageOverviewPanel().find('.sfEditImagePropsUploadedDate').html(value);
    },

    _displayAlbum: function (value) {
        this.get_extendedDetailsPanel().find('.sfEditImagePropsAlbum').html(value);
        this.get_librarySelectorField().control.reset();
    ...
    _displayTitle: function (value) {
        this.get_extendedDetailsPanel().find('.sfEditImagePropsTitle').val(value);
    },

    _displayAltText: function (value) {
        this.get_extendedDetailsPanel().find('.sfEditImagePropsAltText').val(value);
    },

    _displayImageSize: function (value) {
        this.get_extendedDetailsPanel().find('.sfEditImagePropsSize').html(value);
    },

    _displayThumbnail: function (image) {
    ...
    _getTitle: function () {
        var title = this.get_extendedDetailsPanel().find('.sfEditImagePropsTitle').val();
        return { PersistedValue: title, Value: title };
    },

    _getAltText: function () {
        var alternativeText = this.get_extendedDetailsPanel().find('.sfEditImagePropsAltText').val();
    ...
    set_saveImagePropertiesDelegate: function (editDelegate) {
        this._updateImagePropertiesDelegate = editDelegate;
    },

    get_serviceBase: function () {
        return this._serviceBase;
    },
    set_serviceBase: function (value) {
        this._serviceBase = value;
    },
    ...
    get_parentItemType: function () {
        return this._parentItemType;
    },
    set_parentItemType: function (value) {
        this._parentItemType = value;
    },

    /* ********************** DOM element getters/setters ********************** */

    get_imageOverviewPanel: function () {
        if (!this._imageOverviewPanel) {
            this._imageOverviewPanel = jQuery('.sfImageOverviewPanel');
        }
        return this._imageOverviewPanel;
    },

    get_extendedDetailsPanel: function () {
        return jQuery(this._extendedDetailsPanel);
    },
    set_extendedDetailsPanel: function (value) {
        this._extendedDetailsPanel = value;
    },
    ...
    get_librarySelectorField: function () {
        return this._librarySelectorField;
    },
    set_librarySelectorField: function (value) {
        this._librarySelectorField = value;
    }
};
