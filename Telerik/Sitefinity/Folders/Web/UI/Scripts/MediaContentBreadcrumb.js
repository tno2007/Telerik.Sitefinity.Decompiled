...
Telerik.Sitefinity.Folders.Web.UI.MediaContentBreadcrumb=function(element){Telerik.Sitefinity.Folders.Web.UI.MediaContentBreadcrumb.initializeBase(this,[element]);
...
Telerik.Sitefinity.Folders.Web.UI.MediaContentBreadcrumb.prototype={initialize:function(){Telerik.Sitefinity.Folders.Web.UI.MediaContentBreadcrumb.callBaseMethod(this,"initialize");
...
},dispose:function(){if(this._itemClickDelegate){var folderData=this.get_folderData();
if(folderData){for(var i=0;
i<folderData.length;
i++){var currentDataItemId=folderData[i].Id;
var folderLink=$get(currentDataItemId);
$removeHandler(folderLink,"click",this._itemClickDelegate);
}}delete this._itemClickDelegate;
}if(this._rootItemClickDeledate){if(this._rootFolderTag){$removeHandler(this._rootFolderTag,"click",this._rootItemClickDeledate);
...
},add_onRootItemSelectCommand:function(delegate){this.get_events().addHandler("onRootItemSelectCommand",delegate);
},remove_onRootItemSelectCommand:function(delegate){this.get_events().removeHandler("onRootItemSelectCommand",delegate);
},dataBind:function(id){if(id==null){jQuery(this.get_element()).hide();
}else{jQuery(this.get_element()).show();
this._dataBindInternal(id);
}},_itemClickHandler:function(sender,args){var itemId=sender.target.getAttribute("id");
var dataItem=this._findDataItem(itemId,this.get_folderData());
if(dataItem){this._raiseOnItemSelectCommand(dataItem);
}},_rootItemClickHandler:function(sender,args){this._raiseOnRootItemSelectCommand(null);
},_raiseOnItemSelectCommand:function(dataItem){var itemEventArgs=dataItem;
var h=this.get_events().getHandler("onItemSelectCommand");
if(h){h(this,itemEventArgs);
}},_raiseOnRootItemSelectCommand:function(dataItem){var itemEventArgs=dataItem;
var h=this.get_events().getHandler("onRootItemSelectCommand");
if(h){h(this,itemEventArgs);
}},_dataBindInternal:function(id){var urlParams={provider:this.get_providerName(),excludeNeighbours:this.get_excludeNeighbours()};
var manager=this.get_clientManager();
var serviceUrl=this._baseServiceUrl+this._methodServiceUrl;
if(id==null||typeof id==="undefined"){id=this.get_itemId();
}manager.InvokeGet(serviceUrl,urlParams,[id],this._dataBindSuccess,this._dataBindFailure,this);
},_dataBindSuccess:function(sender,result){var folders=result.Items;
sender._buildBreadcrumb(sender,folders);
},_dataBindFailure:function(error,caller){alert(error.Detail);
},_buildBreadcrumb:function(sender,folders){this.get_folderData().length=0;
while(sender._wrapper.hasChildNodes()){sender._wrapper.removeChild(sender._wrapper.lastChild);
...
for(var i=0;
i<folders.length-1;
i++){var currentDataItem=folders[i];
...
sender._wrapper.appendChild(folderTag);
},_findDataItem:function(id,folderData){for(var i=0;
i<folderData.length;
i++){var currentDataItem=folderData[i];
if(currentDataItem.Id==id){return currentDataItem;
}}return null;
...
},set_methodServiceUrl:function(value){this._methodServiceUrl=value;
},get_methodServiceUrl:function(){return this._methodServiceUrl;
...
},get_excludeNeighbours:function(){return this._excludeNeighbours;
},set_excludeNeighbours:function(value){this._excludeNeighbours=value;
},get_folderData:function(){if(!this._folderData){this._folderData=new Array();
}return this._folderData;
},get_rootFolderName:function(){return this._rootFolderName;
},set_rootFolderName:function(value){this._rootFolderName=value;
}};
