function FlatTaxonListExtensions_ViewLoaded(masterGridView){this.itemsGrid=masterGridView.get_itemsGrid();
...
this.initialize=function(){this._itemsGridDataBoundDelegate=Function.createDelegate(this,this.onItemsGridDataBound);
...
this.onItemsGridDataBound=function(sender,args){var dataItem=args.get_dataItem();
var title=dataItem.Title;
var firstLetter=title.substring(0,1);
var row=args.get_itemElement();
if(row){var cell=row.cells[0];
if(cell){var span=jQuery(cell).find("span").get(0);
if(this.letters.indexOf(firstLetter.toUpperCase())==-1){Array.add(this.letters,firstLetter.toUpperCase());
...
}else{if(span!=null){cell.removeChild(span);
}}}cell=row.cells[row.cells.length-1];
if(cell){var itemsCount=dataItem.ItemsCount;
var html=null;
if(itemsCount==0){html=itemsCount+" "+this.clientLabelManager.getLabel("Labels","Items");
jQuery(cell).find("a.sf_binderCommand_viewMarkedItems").parent().html(html);
}else{if(itemsCount==1){html=itemsCount+" "+this.clientLabelManager.getLabel("Labels","Item");
jQuery(cell).find("a.sf_binderCommand_viewMarkedItems").html(html);
}}}}if(sender._itemsBoundCount==sender._itemsToBindCount){this.letters=[];
}};
this.onItemsGridDataBinding=function(sender,args){this.letters=[];
};
