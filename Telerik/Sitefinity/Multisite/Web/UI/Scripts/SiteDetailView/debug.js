...
Telerik.Sitefinity.Multisite.Web.UI.SiteDetailView = function (element) {
    Telerik.Sitefinity.Multisite.Web.UI.SiteDetailView.initializeBase(this, [element]);
...
Telerik.Sitefinity.Multisite.Web.UI.SiteDetailView.prototype = {
    initialize: function () {
        Telerik.Sitefinity.Multisite.Web.UI.SiteDetailView.callBaseMethod(this, "initialize");

        siteDetailView = this;
    },

    dispose: function () {
        Telerik.Sitefinity.Multisite.Web.UI.SiteDetailView.callBaseMethod(this, "dispose");
    },

    reset: function () {
        this.get_propertiesView().hide();
        this.get_configureModulesView().hide();
    },

    show: function (isCreateMode, siteId, configureModules, propertiesView) {
        this.reset();

        if (configureModules) {
            this.get_configureModulesView().set_propertiesView(propertiesView)
            this.get_configureModulesView().show(siteId);
            if (this.get_isInStandaloneMode()) {
                this.get_configureModulesView().arrangeUIForIntegrationMode();
            }

        }
        else {
            this._isIntegratedApp = jQuery("html").hasClass("-sf-integrated-app") || this._isIntegratedApp;
            if (this._isIntegratedApp) {
                jQuery("html").removeClass("-sf-integrated-app");
            }
            ...
            if (this.get_isInStandaloneMode()) {
                this.get_propertiesView().arrangeUIForIntegrationMode();
            }
        }
        ...
        if (!this.get_isInStandaloneMode()) {
            ...
            this.get_kendoWindow().element.parent().addClass("sfWindowAsPartOfPage");
        }
    ...
    close: function (data) {
        if (this.get_backLinkUrl()) {
            window.location.href = this.get_backLinkUrl();
        } else {
            if (this._isIntegratedApp) {
                jQuery("html").addClass("-sf-integrated-app");
                this._isIntegratedApp = false;
            }
    ...
    get_configureModulesView: function () {
        return this._configureModulesView;
    },
    set_configureModulesView: function (value) {
        this._configureModulesView = value;
    },
    ...
    get_isInStandaloneMode: function () {
        return this._isInStandaloneMode;
    },
    set_isInStandaloneMode: function (value) {
        this._isInStandaloneMode = value;
    },
    ...
    set_kendoLoadedHandler: function (value) {
        this._kendoLoadedHandler = value;
    },
    _documentReadyHandler: function () {
        ...
        if (this._kendoLoadedHandler) {
            this._kendoLoadedHandler();
        }
    },
};
