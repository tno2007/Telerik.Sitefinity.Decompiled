...
Telerik.Sitefinity.Multisite.Web.UI.ConfigureModulesView = function (element) {
    Telerik.Sitefinity.Multisite.Web.UI.ConfigureModulesView.initializeBase(this, [element]);
...
Telerik.Sitefinity.Multisite.Web.UI.ConfigureModulesView.prototype = {
    initialize: function () {
        Telerik.Sitefinity.Multisite.Web.UI.ConfigureModulesView.callBaseMethod(this, "initialize");
    ...
    dispose: function () {
        ...
        if (this._startMonitoringSiteInitializationProgressDelegate) {
            delete this._startMonitoringSiteInitializationProgressDelegate;
        }
        if (this._checkSiteInitializationStatusCallbackDelegate) {
            delete this._checkSiteInitializationStatusCallbackDelegate;
        }
        if (this._monitorSiteInitializationProgressDelegate) {
            delete this._monitorSiteInitializationProgressDelegate;
        }
        ...
        if (this._dataSourceProviderSelectorDialogCloseDelegate) {
            if (this.get_dataSourceProviderSelectorDialog()) {
                this.get_dataSourceProviderSelectorDialog().remove_close(this._dataSourceProviderSelectorDialogCloseDelegate);
            }
        ...
        if (this._allModulesCheckboxClickDelegate) {
            if (this.get_allModulesCheckbox()) {
                $removeHandler(this.get_allModulesCheckbox(), "click", this._allModulesCheckboxClickDelegate);
            }
            delete this._allModulesCheckboxClickDelegate;
        }

        if (this._createButtonClickDelegate) {
            if (this.get_createButton()) {
                $removeHandler(this.get_createButton(), "click", this._createButtonClickDelegate);
            }
    ...
    show: function (siteId, siteConfiguration) {
        ...
        if (this._siteConfiguration) {
            var that = this;
            var getSourcesSuccess = function (data) {
                var dataSources = data.filter(function (e) { return e.Name != that.usersDataSourceName; });
                that.usersDataSource = data.find(function (e) { return e.Name == that.usersDataSourceName; });

                if (that._siteConfiguration.DataSources && that._siteConfiguration.DataSources.length === dataSources.length) {
                    that._updateSamples(dataSources);
                }
                else {
                    that._siteConfiguration.DataSources = dataSources;
                }
                that._updateUi();
            };
            ...
            var id = this._siteConfiguration ? this._siteConfiguration.Id : siteId;
            this._rebind(id);
        }
    },

    hide: function () {
        jQuery(this.get_configureModulesViewWrapper()).hide();
    },
    close: function (sender, args) {

        if (this._isCreateMode || this.get_propertiesView()) {
            this.hide();
    ...
    _updateSamples: function (dataSources) {
        for (var i = 0; i < dataSources.length; i++) {
            for (var j = 0; j < this._siteConfiguration.DataSources[i].Links.length; j++) {
                if (this._siteConfiguration.DataSources[i].Links[j].ProviderName === this._siteConfiguration.DataSources[i].SampleLink.ProviderName) {
                    this._siteConfiguration.DataSources[i].Links[j].ProviderName = dataSources[i].SampleLink.ProviderName;
    ...
    _rebind: function (siteId) {
        var that = this;

        if (!this._siteConfiguration) {
            var getSiteConfigSuccess = function (data) {
                that.usersDataSource = data.DataSources.find(function (e) { return e.Name == that.usersDataSourceName; });
    ...
    _updateUi: function () {
        ...
        if (this.usersDataSource) {
            $(this.userSourcesGridElement).data("kendoGrid").setDataSource(new kendo.data.DataSource({
                data: this.usersDataSource.Links,
                sort: { field: "ProviderTitle", dir: "asc" }
            }));
            labelKey = "ConfigureModulesAndAccessFor";
            this.userGroupsSectionElement.show();
        } else {
            labelKey = "ConfigureModulesFor";
            this.userGroupsSectionElement.hide();
        }

        if (this._isCreateMode) {
            jQuery(this.get_backLink()).html(this.get_clientLabelManager().getLabel("MultisiteResources", "BackToCreateSite"));
            jQuery(this.get_createButton()).show();
            jQuery(this.get_saveButton()).hide();
        } else {
            if (this.get_propertiesView()) {
                jQuery(this.get_backLink()).html(this.get_clientLabelManager().getLabel("MultisiteResources", "BackToSiteProperties"));
            } else {
                jQuery(this.get_backLink()).html(this.get_clientLabelManager().getLabel("MultisiteResources", "BackToSites"));
            }
        ...
        for (var i = 0; i < this._siteConfiguration.DataSources.length; i++) {
            if (this._siteConfiguration.DataSources[i].Links) {
                this._siteConfiguration.DataSources[i].Links.sort(function (a, b) {
                    var aVal = a.ProviderTitle ? a.ProviderTitle : a.ProviderName;
        ...
        if (!this._isCreateMode) {
            var isReadMode = this._siteConfiguration.SiteConfigurationMode === Telerik.Sitefinity.Multisite.Web.Services.SiteConfigurationMode.ConfigureByDeployment;
    ...
    _switchDisplayMode: function (isReadMode) {
        if (isReadMode) {
            jQuery(this._element).find("input").prop('disabled', true);
            ...
            jQuery(this._element).find("input").prop('disabled', false);
    ...
    _getCheckboxCell: function (dataItem) {
        var jCell = jQuery("<td />");
        ...
        var that = this;
        var checkboxClick = function (sender, dataSourceName) {
            return function () {
                dataItem.IsChecked = sender.is(":checked");
                //check dependent data sources together with this one
                if (dataItem.DependantDataSources && dataItem.DependantDataSources.length !== 0) {
                    for (var i = 0; i < that._siteConfiguration.DataSources.length; i++) {
                        var otherItem = that._siteConfiguration.DataSources[i];
                        for (var j = 0; j < dataItem.DependantDataSources.length; j++) {
                            if (otherItem.Name === dataItem.DependantDataSources[j]) {
                                otherItem.IsChecked = sender.is(":checked");
                            }
                        }
                    }
                }
    ...
    _getLabelCell: function (dataItem) {
        var jCell = jQuery("<td />");
    ...
    _getDataSourceCheckboxIdPrefix: function () {
        return this.get_id() + "_datasource_checkbox_";
    },

    _getDataSourceCheckboxId: function (dataSourceName) {
        return this._getDataSourceCheckboxIdPrefix() + dataSourceName.replace(/\.|\s/g, "_");
    },

    _getSiteDataSourceLinksCell: function (dataItem) {
        var jCell = jQuery("<td />");
        ...
        if (!dataItem.IsChecked) {
            ...
            if (dataItem.Links.length === 0 && dataItem.SampleLink) {
                dataItem.Links = [dataItem.SampleLink];
            }
        ...
        var that = this;
        jChangeAnchor.click(function () {
            that.get_dataSourceProviderSelectorDialog().show(that._siteConfiguration, dataItem);
        });
    ...
    _getTitles: function (links) {
        var result = [];
        for (var i = 0; i < links.length; i++) {
            var title = links[i].ProviderTitle ? links[i].ProviderTitle : links[i].ProviderName;
    ...
    _getChangeAnchorId: function (dataSourceName) {
        return this.get_id() + "_change_" + dataSourceName.replace(/\./g, "_");
    },

    _getSiteConfiguration: function (siteId, onSuccess) {
        this._setLoadingViewVisible(true);
        jQuery.ajax({
            type: 'GET',
            url: this.get_webServiceUrl() + String.format('{0}/config/', siteId),
    ...
    _getDataSources: function (onSuccess) {
        this._setLoadingViewVisible(true);
        jQuery.ajax({
            type: 'GET',
            url: this.get_webServiceUrl() + 'config/sources/',
            data: jQuery.param({ siteName: this._siteConfiguration.Name }),
            contentType: "application/json",
            processData: false,
            success: onSuccess,
            error: this._ajaxFailDelegate,
    ...
    _ajaxFailHandler: function (jqXHR, textStatus, errorThrown) {
        this._stopMonitoringSiteInitializationProgress();
    ...
    _setLoadingViewVisible: function (loading) {
        ...
        if (this._loadingCounter > 0) {
            ...
            if (!this._siteConfiguration || this._siteConfiguration.SiteConfigurationMode !== Telerik.Sitefinity.Multisite.Web.Services.SiteConfigurationMode.ConfigureByDeployment) {
    ...
    _save: function (sender, args) {
        this._saveSiteContentSources();
    },

    _createButtonClickHandler: function (sender, args) {
        this._createSite();
    },
    ...
    _createSite: function () {
        ...
        jQuery.ajax({
            type: 'PUT',
            url: this.get_webServiceUrl() + "/createSiteAsync/",
            contentType: "application/json",
            processData: false,
            data: this._getSiteConfigurationJsonModel(),
            success: this._startMonitoringSiteInitializationProgressDelegate,
            error: this._ajaxFailDelegate
        });
    },

    _saveSiteContentSources: function () {
        this._setLoadingViewVisible(true);
        var that = this;
        jQuery.ajax({
            type: 'PUT',
            url: this.get_webServiceUrl() + "sources/" + this._siteConfiguration.Id,
            contentType: "application/json",
            processData: false,
            data: this._getSiteConfigurationJsonModel(),
            success: function () {
    ...
    _getSiteConfigurationJsonModel: function () {
        if (this.usersDataSource) {
            this.usersDataSource.IsChecked = true;
    ...
    _startMonitoringSiteInitializationProgress: function () {
        this._monitorSiteInitializationProgress();
        this._checkProgress = setInterval(this._monitorSiteInitializationProgressDelegate, 2000);
    },

    _stopMonitoringSiteInitializationProgress: function () {
        clearInterval(this._checkProgress);
        this._checkProgress = null;
    },

    _monitorSiteInitializationProgress: function () {
        jQuery.ajax({
            type: 'GET',
            url: this.get_webServiceUrl() + "checkSiteInitializationStatus/",
            contentType: "application/json",
            processData: false,
            data: jQuery.param({ siteName: this._siteConfiguration.Name }),
            success: this._checkSiteInitializationStatusCallbackDelegate,
            error: this._ajaxFailDelegate
        });
    },

    _checkSiteInitializationStatusCallback: function (data) {
        if (!data.IsInProgress) {
            this._stopMonitoringSiteInitializationProgress();

            if (data.ErrorMessage == '' || data.ErrorMessage == null) {
                siteDetailView.close("reload");
            } else {
                this.get_messageControl().showNegativeMessage(data.ErrorMessage);
            }
    ...
    _dataSourceProviderSelectorDialogCloseHandler: function (sender, args) {
    ...
    _getSiteDataSourceLinkIdx: function (moduleName, providerName) {
        for (var i = 0; i < this._site.ContentSources.length; i++) {
            if (this._site.ContentSources[i].ModuleName === moduleName && this._site.ContentSources[i].ProviderName === providerName) {
                return i;
            }
        }
        return -1;
    ...
    _back: function (sender, args) {
        this.close();
    },

    _allDataSourcesCheckboxClickHandler: function (sender, args) {
        for (var i = 0; i < this._siteConfiguration.DataSources.length; i++) {
            this._siteConfiguration.DataSources[i].IsChecked = this.get_allDataSourcesCheckbox().checked;
        }
        this._updateUi();
    },

    _getDefaultSiteDataSourceLinkTitle: function (dataSourceTitle) {
        return this._site.Name + " " + dataSourceTitle;
    },
    ...
    _changeConfigurationMode: function (newMode) {
        if (this._siteConfiguration.SiteConfigurationMode !== newMode) {
            var that = this;
            var onSuccess = function (data) {
                that._siteConfiguration.SiteConfigurationMode = data.Mode;
                that._updateUi();
            };
    ...
    initializeUserGroups: function () {
        ...
        $("#changeUserGroupsButton").click(function () {
            that.get_dataSourceProviderSelectorDialog().show(that._siteConfiguration, that.usersDataSource);
        });

        $(this.userSourcesGridElement).kendoGrid({
            columns: [
                {
                    field: "ProviderTitle",
                    title: " ",
                    template: "#:ProviderTitle# #if(IsGlobalProvider) {#<span class='sfLightTxt'>" + that.get_clientLabelManager().getLabel("MultisiteResources", "GlobalUserGroupNote") + "</span>#}#"
                },
                {
                    template: "<span class='sfDeleteProfileOn'><i class='fa fa-trash-o' title='" + that.get_clientLabelManager().getLabel("MultisiteResources", "RemoveFromThisSite") + "'></i></span>",
                    attributes: {
                        "class": "sfAlignCenter sfEditCol"
                    }
                }
            ],
            scrollable: false,
            dataBound: jQuery.proxy(this._usersDataSourceBound, this)
        });

        $(this.userSourcesGridElement).on("click", "span.sfDeleteProfileOn", function () {
            var row = $(this).closest("tr");
            var grid = $(that.userSourcesGridElement).data("kendoGrid");
            var dataItem = grid.dataItem(row);
            grid.dataSource.remove(dataItem);
            that.usersDataSource.Links = that.usersDataSource.Links.filter(function (item) {
                return item.ProviderName !== dataItem.ProviderName;
            });
        });
    },

    _usersDataSourceBound: function (arg) {
        var that = this;

        if (arg.sender._data.length == 1) {
            arg.sender.hideColumn(arg.sender.columns[1]);
        } else {
            arg.sender.showColumn(arg.sender.columns[1]);
        }
    },
    ...
    get_configureModulesViewWrapper: function () {
        return this._configureModulesViewWrapper;
    },
    set_configureModulesViewWrapper: function (value) {
        this._configureModulesViewWrapper = value;
    },
    ...
    get_dataSourcesTableBody: function () {
        return this._dataSourcesTableBody;
    },
    set_dataSourcesTableBody: function (value) {
        this._dataSourcesTableBody = value;
    },
    ...
    get_dataSourceProviderSelectorDialog: function () {
        return this._dataSourceProviderSelectorDialog;
    },
    set_dataSourceProviderSelectorDialog: function (value) {
        this._dataSourceProviderSelectorDialog = value;
    },
    get_propertiesView: function () {
        return this._propertiesView;
    },
    set_propertiesView: function (value) {
        this._propertiesView = value;
    },
    get_allDataSourcesCheckbox: function () {
        return this._allDataSourcesCheckbox;
    },
    set_allDataSourcesCheckbox: function (value) {
        this._allDataSourcesCheckbox = value;
    },
