...
function OnMasterViewLoaded(sender,args){sidebar=sender.get_sidebar();
...
}function handleItemsGridCommand(sender,args){switch(args.get_commandName()){case"activeFeeds":itemsGrid.applyFilter("IsActive == true");
break;
...
break;
default:break;
}}function handleBinderItemCommand(sender,args){var commandName=args.get_commandName();
var dataItem=args.get_dataItem();
var itemElement=args.get_itemElement();
switch(commandName){case"startUpdating":case"stopUpdating":var setActive=commandName==="startUpdating";
...
break;
default:break;
}}function successCallback(caller,data,request,context){itemsGrid.dataBind();
}function failureCallback(error,caller,context){alert(error.Detail);
...
jQuery.each(pipes,function(i,pipe){jQuery("<li />").appendTo(list).addClass("sfPipeTitle sfPipe"+pipe.PipeName).html(pipe.UIName);
});
...
if(dataItem.IsBackend){itemsGrid.removeActionsMenuItems([actionCommandPrefix+"delete"],args.get_itemElement());
if(!dataItem.IsActive){itemsGrid.removeActionsMenuItems([actionCommandPrefix+"deactivate"],args.get_itemElement());
}}else{itemsGrid.removeActionsMenuItems([actionCommandPrefix+"deactivate"],args.get_itemElement());
}$("#"+args._itemElement.id).find(".sfProgressTaskCommand").click(function(){hideProcessingLoader(args._itemElement.id);
itemsGrid.dataBind();
});
itemToRowMapping[dataItem.Id]=args._itemElement.id;
}function handleBinderDataBound(sender,args){monitorReindexingProgress();
}function showProcessingLoader(itemID,progress,statusMessage){var itemEl=jQuery("#"+itemID+" .sfDateAuthor");
...
if(progress>=100){statusMessage=itemEl.find(".resIndexingContent").html();
...
if(progress>=100){doneBtn.show();
}else{doneBtn.hide();
}actionsEl.hide();
wrapper.show();
}function hideProcessingLoader(itemID){var itemEl=jQuery("#"+itemID+" .sfDateAuthor");
var actionsEl=jQuery("#"+itemID+" .sfMoreActions .actionsMenu");
itemEl.find(".sfLine").removeClass("sfDisplayNoneImportant");
itemEl.find(".sfProgress").hide();
actionsEl.show();
}function startMonitoringReindexProgress(){monitorReindexingProgress();
checkProgress=setInterval(monitorReindexingProgress,2000);
}function stopMonitoringReindexProgress(){clearInterval(checkProgress);
checkProgress=null;
}function monitorReindexingProgress(){var serviceUrl=serviceBaseUrl+"/getReindexStatus/";
clientManager.InvokeGet(serviceUrl,{},{},reindexMonitorSuccessCallback);
}function reindexMonitorSuccessCallback(caller,data){if(!Array.isArray(data)){return;
}var reindexedPointsToRemove=Object.keys(currentlyReindexedPoints);
currentlyReindexedPoints={};
for(var i=0;
i<data.length;
i++){var item=data[i].Key;
...
i++){var itemToRemove=reindexedPointsToRemove[i];
if(!currentlyReindexedPoints.hasOwnProperty(itemToRemove)){showProcessingLoader(itemToRowMapping[itemToRemove],100);
shouldRebind=true;
}}}