...
Telerik.Sitefinity.Modules.Libraries.Web.UI.Images.ImageEditorDialog = function (element) {
    Telerik.Sitefinity.Modules.Libraries.Web.UI.Images.ImageEditorDialog.initializeBase(this, [element]);
...
Telerik.Sitefinity.Modules.Libraries.Web.UI.Images.ImageEditorDialog.prototype = {
    initialize: function () {
        Telerik.Sitefinity.Modules.Libraries.Web.UI.Images.ImageEditorDialog.callBaseMethod(this, "initialize");
    ...
    organizeToolGroups: function () {
        var groups = this.get_imageEditor().get_toolGroups();
        for (var i = 0; i < groups.length; i++) {
            var group = groups[i];
            if (jQuery.grep(group.get_allItems(), this._isZoomCommand)) {
                jQuery(group.get_element()).css({ "float": "right" });
                break;
            }
        }
    },
    ...
    _attachImageEditorHandlers: function (toAttach) {
        var imageEditor = this.get_imageEditor();
        if (toAttach) {
            this._imageChangedDelegate = Function.createDelegate(this, this._imageChanged);
            ...
            jQuery.extend(editableImage, {
                _finishSave: this._savingDelegate,
                _finishOperation: this._fixImageOnSaveDelegate,
                set_serverUrl: this._saveSuccessDelegate
            });
        }
        else {
            ...
            if (this._imageChangedDelegate) {
                imageEditor.remove_imageChanging(this._imageChangedDelegate);
                delete this._imageChangedDelegate;
            }

            if (this._commandExecutingDelegate) {
                imageEditor.remove_commandExecuting(this._commandExecutingDelegate);
    ...
    _attachButtonHandlers: function (toAttach) {
        ...
        if (toAttach) {
            ...
            if (saveAsButton) {
                this._saveAsButtonClickDelegate = Function.createDelegate(this, this._saveAsButtonClick);
                $addHandler(saveAsButton, "click", this._saveAsButtonClickDelegate);
            }
        }
        else {
            if (this._cancelButtonClickDelegate) {
                $removeHandler(cancelButton, "click", this._cancelButtonClickDelegate);
                delete this._cancelButtonClickDelegate;
            }

            if (this._saveButtonClickDelegate) {
                $removeHandler(saveButton, "click", this._saveButtonClickDelegate);
                delete this._saveButtonClickDelegate;
            }

            if (this._saveAsButtonClickDelegate) {
                $removeHandler(saveAsButton, "click", this._saveAsButtonClickDelegate);
                delete this._saveAsButtonClickDelegate;
            }
        }
    },

    _saveButtonClick: function (args) {
        if (!jQuery(this.get_saveButton()).hasClass("sfDisabledLinkBtn")) {
            this._imageWidth = this.get_imageEditor().getEditableImage().get_width();
            this._imageHeight = this.get_imageEditor().getEditableImage().get_height();
            this.get_imageEditor().saveImageOnServer("", true);

        }
    },
    _saveAsButtonClick: function (args) {
        if (!jQuery(this.get_saveAsButton()).hasClass("sfDisabledLinkBtn")) {
            this.get_imageEditor().fire("SaveAs");
        }
    },
    _cancelButtonClick: function (args) {
        this._closeDialog("cancel");
    },
    _saving: function () {
        this._isSave = true;
    },
    _saveSuccess: function (guid, args) {
        this.get_imageEditor().getEditableImage()._serverUrl = guid;
        if (this._isSave) {
            this.set_imageId(guid);
            this._isSave = false;

            var x;
            if (args.indexOf("|") !== -1) {
			    x = args.split('|');
            } else if (args.indexOf("%7C") !== -1) {
                x = args.split("%7C");
            } else if (args.indexOf("%7c") !== -1) {
                x = args.split("%7c");
            } else {
    ...
    _fixImageOnSave: function (serverData, context, callback) {
        var editableImage = this.get_imageEditor().getEditableImage();
        if (context.data.name == "Save") {
            var data = editableImage._readJson(serverData);

            if (typeof (callback) == "function")
                callback.call(editableImage, context.data);

            if (data.serverUrl)
                editableImage.set_serverUrl(data.serverUrl, data.args);

            //display message if such is present
            editableImage._alertMessage(data.alertMessage);
        }
        else
            this._originalEditableImageFinishOp.apply(editableImage, arguments);
    },
    _commandExecuting: function (sender, args) {
        var cmdName = args.get_commandName();
        if (cmdName == "Crop" || cmdName == "SaveAs" || cmdName == "Resize") {
            var hiddenInput = $get(sender.get_clientStateFieldID());
    ...
    _imageChanged: function (sender, args) {
        if (args.get_commandName() != "Zoom") this._enableSaveButtons(args.get_commandName() != "Reset");
    },
    _enableSaveButtons: function (toEnable) {
        if (toEnable) {
            jQuery(this.get_saveButton()).removeClass("sfDisabledLinkBtn").addClass("sfSave");
            jQuery(this.get_saveAsButton()).removeClass("sfDisabledLinkBtn");
        }
        else {
            jQuery(this.get_saveButton()).addClass("sfDisabledLinkBtn").removeClass("sfSave");
            jQuery(this.get_saveAsButton()).addClass("sfDisabledLinkBtn");
        }
    },
    _closeDialog: function (value, params) {
        if (value == "save") {
        	this.close({ CommandName: value, ImageId: this.get_imageId(), DisableHistory: true, ProviderName: params.ProviderName, SaveArgument : params.SaveArgument, Width: this._imageWidth, Height: this._imageHeight });
            return;
        }
    ...
    _closeDialogEnded: function (sender, args) {
        sender.remove_responseEnding(this._closeDialogEndedDelegate);
    ...
    //gets the Id of the image being edited
