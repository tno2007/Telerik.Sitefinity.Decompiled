...
Telerik.Sitefinity.Multisite.Web.UI.ShareItemDialog=function(element){Telerik.Sitefinity.Multisite.Web.UI.ShareItemDialog.initializeBase(this,[element]);
...
Telerik.Sitefinity.Multisite.Web.UI.ShareItemDialog.prototype={initialize:function(){Telerik.Sitefinity.Multisite.Web.UI.ShareItemDialog.callBaseMethod(this,"initialize");
...
},_doneButtonClickHandler:function(sender,args){var that=this;
var doneSuccess=function(data,textStatus,jqXHR){that.closeUpdated();
};
this._setLoadingViewVisible(true);
jQuery.ajax({type:"PUT",url:this._setSharedSitesUrl,contentType:"application/json",processData:false,data:Sys.Serialization.JavaScriptSerializer.serialize(this._getSharedSiteIds()),success:doneSuccess,error:this._ajaxFailDelegate,complete:this._ajaxCompleteDelegate});
},_getSharedSiteIds:function(){var result=[];
for(var i=0;
i<this._data.length;
i++){if(this._data[i].IsShared){result.push(this._data[i].SiteId);
}}return result;
},_sitesGridDataBoundHandler:function(sender,args){if(this._data){for(var i=0;
i<this._data.length;
i++){var jCheckbox=jQuery("#"+this._siteCheckboxId(this._data[i].SiteId));
var that=this;
var checkboxClick=function(checkbox,idx){return function(){that._data[idx].IsShared=checkbox.is(":checked");
};
}(jCheckbox,i);
jCheckbox.click(checkboxClick);
}}},_loadSites:function(){var that=this;
var loadSitesSuccess=function(data,textStatus,jqXHR){if(data.Items){for(var i=0;
i<data.Items.length;
i++){data.Items[i].SiteName=data.Items[i].SiteName.htmlEncode();
}}that._data=data.Items;
that._initializeGrid();
};
this._setLoadingViewVisible(true);
jQuery.ajax({type:"GET",url:this._getSharedSitesUrl,contentType:"application/json",processData:false,success:loadSitesSuccess,error:this._ajaxFailDelegate,complete:this._ajaxCompleteDelegate});
},_initializeGrid:function(){var dataSource=new kendo.data.DataSource({data:this._data});
...
},_siteCheckboxId:function(siteId){return this.get_id()+"_siteCheckbox_"+siteId;
},_siteRowId:function(siteId){return this.get_id()+"_siteRow_"+siteId;
...
},_searchBoxSearchHandler:function(sender,args){if(this._data){var query=args.get_query();
if(query){query=query.toLowerCase();
}for(var i=0;
i<this._data.length;
i++){var jRow=jQuery("#"+this._siteRowId(this._data[i].SiteId));
if(query&&this._data[i].SiteName.toLowerCase().indexOf(query)===-1){jRow.hide();
}else{jRow.show();
