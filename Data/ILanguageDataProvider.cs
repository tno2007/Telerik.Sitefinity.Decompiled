// Decompiled with JetBrains decompiler
// Type: Telerik.Sitefinity.Data.ILanguageDataProvider
// Assembly: Telerik.Sitefinity, Version=13.3.7600.0, Culture=neutral, PublicKeyToken=b28c218413bdf563
// MVID: 39C4B52A-B559-4D9C-97D9-CCCF73C3738E
// Assembly location: C:\Programs\Sitefinity\ProjectManager_13_3_7600\_EmptyProject\bin\Telerik.Sitefinity.dll

using System;
using System.Linq;
using Telerik.Sitefinity.Lifecycle;

namespace Telerik.Sitefinity.Data
{
  /// <summary>
  /// Interface for providers that will rely on Language Data objects (store language specifc data)
  /// </summary>
  public interface ILanguageDataProvider
  {
    /// <summary>Creates a language data item</summary>
    /// <returns></returns>
    LanguageData CreateLanguageData();

    /// <summary>Creates a language data item</summary>
    /// <param name="id">The id.</param>
    /// <returns></returns>
    LanguageData CreateLanguageData(Guid id);

    /// <summary>Gets language data item by its id</summary>
    /// <param name="id">The id.</param>
    /// <returns></returns>
    LanguageData GetLanguageData(Guid id);

    /// <summary>Gets a query of all language data items</summary>
    /// <returns></returns>
    IQueryable<LanguageData> GetLanguageData();
  }
}
