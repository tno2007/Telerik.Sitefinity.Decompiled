...
Telerik.Sitefinity.Modules.Forms.Web.UI.SiteSelectorDialog=function(element){Telerik.Sitefinity.Modules.Forms.Web.UI.SiteSelectorDialog.initializeBase(this,[element]);
...
Telerik.Sitefinity.Modules.Forms.Web.UI.SiteSelectorDialog.prototype={initialize:function(){siteSelectorDialog=this;
...
},dispose:function(){if(this._doneClientSelectionDelegate){if(this._doneButton){$removeHandler(this._doneButton,"click",this._doneClientSelectionDelegate);
}delete this._allSitesDoneSelectingDelegate;
}if(this._cancelClientsSelectionDelegate){if(this._cancelButton){$removeHandler(this._cancelButton,"click",this._cancelClientsSelectionDelegate);
...
},_onLoad:function(){this.get_siteSelector().set_clearSelectionOnRebind(false);
this.get_siteSelector().set_selectedKeys(this.exisitngLinks);
jQuery("body").addClass("sfSelectorDialog");
dialogBase.resizeToContent();
},_doneClientSelection:function(items){if(items){var selectedKeys=this.get_siteSelector().get_selectedKeys();
var deselectedKeys=this.get_siteSelector().get_deselectedKeys();
var key=null;
for(var i=0;
i<selectedKeys.length;
i++){key=selectedKeys[i];
if(this.exisitngLinks.indexOf(key)===-1){this.exisitngLinks.push(key);
}}for(var i=0;
i<deselectedKeys.length;
i++){key=deselectedKeys[i];
var idx=this.exisitngLinks.indexOf(key);
if(idx!==-1){this.exisitngLinks.splice(idx,1);
}}var data={formId:this.formId,selectedSites:this.exisitngLinks};
this._save(data);
}else{this.close();
}},_cancelClientsSelection:function(){this.close();
},_setTitle:function(){var titleElement=this._siteSelector.get_lblTitle();
if(titleElement){var itemsName=null;
var selectedItemsCount=0;
var args=this.get_radWindow()._sfArgs;
if(args){var itemsList=args.get_itemsList();
if(itemsList){selectedItemsCount=itemsList.get_selectedItems().length;
}}var contentTypeName=this._siteSelector.get_contentTypeName();
if(selectedItemsCount==1){itemsName=this._siteSelector.get_itemName();
}else{itemsName=this._siteSelector.get_itemsName();
...
$.ajax({url:that.webServiceUrl,type:"POST",contentType:"application/json",data:JSON.stringify(data),success:function(data){that._hideLoadingSection();
that.closeAndRebind();
