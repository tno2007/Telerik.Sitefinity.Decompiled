...
Telerik.Sitefinity.Web.UI.ContentUI.Views.Backend.MasterDetailView = function (element) {
    Telerik.Sitefinity.Web.UI.ContentUI.Views.Backend.MasterDetailView.initializeBase(this, [element]);
...
Telerik.Sitefinity.Web.UI.ContentUI.Views.Backend.MasterDetailView.prototype =
{
    initialize: function () {
        this._handlePageLoadDelegate = Function.createDelegate(this, this._pageLoad);
    ...
    dispose: function () {
        if (this._masterView && this._masterView.get_currentItemsList()) {
            this._masterView.get_currentItemsList().remove_selectionChanged(this._masterViewSelectionChangedDelegate);
        }
    ...
    add_masterViewSelectionChanged: function (delegate) {
        this.get_events().addHandler('masterViewSelectionChanged', delegate);
    },

    // Happens when selection of the pages is changed (node is checked or unchecked)
    remove_masterViewSelectionChanged: function (delegate) {
        this.get_events().removeHandler("masterViewSelectionChanged", delegate);
    },

    /* -------------------- events -------------------- */

    _raiseMasterViewSelectionChanged: function (selectedItems) {
    ...
    _pageLoad: function (sender, args) {
        this._masterView.add_selectionChanged(this._masterViewSelectionChangedDelegate);
    },
    ...
    _masterViewSelectionChanged: function (sender, eventArgs) {
        ...
        if (selectedItemsCount > 0) {
            if (this._currentMasterSelectedItem != selectedItems[0]) {
                this._detailView.reset();
    ...
    set_detailView: function (value) {
        if (value !== this._detailView) {
    ...
    set_masterView: function (value) {
        if (value !== this._masterView) {
            this._masterView = value;
        }
    }
};
