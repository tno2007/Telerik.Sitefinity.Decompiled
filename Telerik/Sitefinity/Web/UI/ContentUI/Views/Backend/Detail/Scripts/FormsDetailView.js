...
Telerik.Sitefinity.Web.UI.ContentUI.Views.Backend.Detail.FormsDetailView=function(element){this._element=element;
...
Telerik.Sitefinity.Web.UI.ContentUI.Views.Backend.Detail.FormsDetailView.prototype={initialize:function(){Telerik.Sitefinity.Web.UI.ContentUI.Views.Backend.Detail.FormsDetailView.callBaseMethod(this,"initialize");
},dispose:function(){if(this._deleteEntryClickDelegate){delete this._deleteEntryClickDelegate;
}if(this._editEntryClickDelegate){delete this._editEntryClickDelegate;
...
for(var keyName in key){var val=key[keyName];
if(typeof val!=="function"){keys.push(val);
}}clientManager.InvokeGet(this._binder.get_serviceBaseUrl(),urlParams,key,this._dataBindSuccess,this._dataBindFailure,this);
},rebind:function(){this._binder.reset();
this.dataBind(this._callerDataItem,this._dataKey);
},reset:function(){this._binder.reset();
},add_detailsChanged:function(delegate){this.get_events().addHandler("detailsChanged",delegate);
},remove_detailsChanged:function(delegate){this.get_events().removeHandler("detailsChanged",delegate);
},_raiseDetailsChanged:function(){var handler=this.get_events().getHandler("detailsChanged");
if(handler){handler(this);
}},add_detailViewCommand:function(delegate){this.get_events().addHandler("detailViewCommand",delegate);
},remove_detailViewCommand:function(delegate){this.get_events().removeHandler("detailViewCommand",delegate);
},_raiseDetailViewCommand:function(commandName,commandArgument){var args=new Telerik.Sitefinity.CommandEventArgs(commandName,commandArgument);
var handler=this.get_events().getHandler("detailViewCommand");
if(handler){handler(this,args);
}},_handlePageLoad:function(sender,args){this._deleteEntryClickDelegate=Function.createDelegate(this,this._deleteEntryClickHandler);
this._deleteButton=$get(this._deleteEntryButtonId);
if(this._deleteButton){$addHandler(this._deleteButton,"click",this._deleteEntryClickDelegate);
...
if(this._editButton){$addHandler(this._editButton,"click",this._editEntryClickDelegate);
...
},_deleteEntryClickHandler:function(e){if(confirm(this._deleteConfirmationMessage)){var keys=new Array();
...
}},_deleteSuccess:function(sender,result){sender._raiseDetailsChanged();
},_deleteFailure:function(result){alert(result.Details);
},_editEntryClickHandler:function(e){this._raiseDetailViewCommand("edit",this.get_binder().get_dataItem());
