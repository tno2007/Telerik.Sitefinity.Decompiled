...
Telerik.Sitefinity.Workflow.UI.WorkflowMenu = function (element) {
    Telerik.Sitefinity.Workflow.UI.WorkflowMenu.initializeBase(this, [element]);
    ...
    // the culture which is used currently by the backend
    this._backendUICulture = null;
    this._contentCulture = null;
    this._workflowWarningDelegate = null;

    // Defines if user friendly message will be shown.
...
Telerik.Sitefinity.Workflow.UI.WorkflowMenu.prototype = {

    // set up 
    initialize: function () {

        this._workflowCommandButtonDelegate = Function.createDelegate(this, this._handleWorkflowButtonCommand);
        ...
        if (!this._messageWorkflowSuccessDelegate) {
    ...
    dispose: function () {
        ...
        if (this._workflowCommandButtonDelegate) {
            delete this._workflowCommandButtonDelegate;
        }

        if (this._workflowMenuMouseOverDelegate) {
            delete this._workflowMenuMouseOverDelegate;
        }

        if (this._workflowCommandMenuDelegate) {
            delete this._workflowCommandMenuDelegate;
        }

        if (this._workflowWindowCloseDialog) {
            delete this._workflowWindowCloseDialog;
        }
        ...
        if (this._messageWorkflowFailureDelegate) {
            delete this._messageWorkflowFailureDelegate;
        }

        if (this._messageWorkflowSuccessDelegate) {
            delete this._messageWorkflowSuccessDelegate;
        }

        if (this._workflowWindowOpenDialogDelegate) {
            delete this._workflowWindowOpenDialogDelegate;
        }
        ...
        if (this._workflowWarningDelegate) {
            if (this.get_workflowWarningDialog()) {
                this.get_workflowWarningDialog().remove_command(this._workflowWarningDelegate);
            }
    ...
    add_onNoWorkflowActions: function (delegate) {
        this.get_events().addHandler('onNoWorkflowActions', delegate);
    },
    remove_onNoWorkflowActions: function (delegate) {
        this.get_events().removeHandler('onNoWorkflowActions', delegate);
    },

    // cancelable event raised upon workflow failure 
    add_onWorkflowFailure: function (delegate) {
        this.get_events().addHandler("onWorkflowFailure", delegate);
    },

    remove_onWorkflowFailure: function (delegate) {
        this.get_events().removeHandler("onWorkflowFailure", delegate);
    },
    ...
    bindWorkflowVisuals: function (itemType, providerName, itemId, dataItem, validationFunction) {
        if (!itemId && dataItem) {
            itemId = dataItem.OriginalContentId;
        }
        ...
        if (this._lastWorkflowItemId) {
            this._initialBinding = itemId != this._lastWorkflowItemId;
        }
        else {
            this._initialBinding = true;
        }
    ...
    operationClosesDialog: function (workflowOperationName) {
        var elements = this._workflowVisualElements;
        for (var i = 0, length = elements.length; i < length; i++) {
            var visualElement = elements[i];
            if (visualElement.OperationName == workflowOperationName) {
                return visualElement.ClosesForm;
            }
        }
        return true;
    },

    rebindWorkflowVisuals: function (lastModifiedItemContext) {
        var lastModifiedItem = lastModifiedItemContext.Item;
    ...
    // Calls the workflow service to get the necessary workflow visuals to display
    _bindWorkflowVisuals: function () {
        ...
        if (this._showMoreActions !== undefined) {
            urlParams["showMoreActions"] = this._showMoreActions;
        }
        if (this._isMultilingual) {
            clientManager.set_uiCulture(this._backendUICulture); // visuals should be bound according to the Backend Culture
            urlParams["itemCulture"] = this.get_contentCulture(); // detection of the status of the item is based on the culture that is used at the moment for editing
        }
        var keys = [this.get_itemId()];
        this._beforeRequest();
        clientManager.InvokeGet(this._workflowServiceUrl, urlParams, keys,
                              this._bindWorkflowVisualsSuccessDelegate, this._bindWorkflowVisualsFailureDelegate, this);
    },

    // cotnext bag should be in the following format [{key:"key1",value:"value1},{key:"key1",value:"value2}]
    _sendMessageWorkflow: function (workflowOperation, contextBag) {
        var clientManager = new Telerik.Sitefinity.Data.ClientManager();
        var urlParams = [];
        urlParams["itemType"] = this.get_itemType();
        urlParams["providerName"] = this.get_providerName();
        ...
        if (this._isMultilingual) { // setting the content culture to be set while processing the message
            clientManager.set_uiCulture(this.get_contentCulture());
        }
        if (contextBag == null) {
            contextBag = [];
        }
        ...
        if (this._validateItem) {
            contextBag.push({ key: this._validateItemKey, value: "true" });
        }
    ...
    // Builds the menu based on the visual element items
    _buildMenu: function (items, workflowDefinition) {
        this._workflowVisualElements = items;
        this._resetVisuals();
        ...
        for (var i = 0, length = items.length; i < length; i++) {
            var visualElementDefinition = items[i];
            if (hidePreviewAction && visualElementDefinition.OperationName == "Preview")
                continue;
            switch (visualElementDefinition.DecisionType) {
                case 'MainAction':
                    mainActions.push(visualElementDefinition);
                    break;
                case 'SecondaryActions':
                    secondaryActions.push(visualElementDefinition);
                    break;
                case 'OtherActions':
                    otherActions.push(visualElementDefinition);
                    break;
                default:
                    alert('Visual element type not supported! ' + visualElementDefinition.DecisionType);
            }
        }

        if (mainActions.length > 0) {
            this._buildActions(mainActions, workflowDefinition);
        }

        if (secondaryActions.length > 0) {
            this._buildActions(secondaryActions, workflowDefinition, this.get_secondaryActionsContainer());
        }

        this._buildOtherActions(otherActions);
    },

    _buildActions: function (actions, workflowDefinition, actionsContainer) {
        if (actionsContainer == null) {
            actionsContainer = this.get_actionsContainer();
        }
        for (var i = 0, length = actions.length; i < length; i++) {
            var actionDefinition = actions[i];
            var visualElement = this._generateVisualElement(actionDefinition);

            if (workflowDefinition != null) {
                jQuery(visualElement).attr("Title", actionDefinition.Title + " (" + workflowDefinition.Title + ")");
            }
            else
                jQuery(visualElement).attr("Title", actionDefinition.Title);
    ...
    _buildOtherActions: function (actions) {
        ...
        // hide more actions menu, if there is no items in there
        if (rootItem.get_items().get_count() <= 0) {
            this.get_otherActionsMenu().set_visible(false);
        } else {
            this.get_otherActionsMenu().set_visible(true);
        }
    },


    // Callback function called if the workflow service successfully returns workflow visuals
    _bindWorkflowVisuals_Success: function (caller, result) {
        ...
        if (result != null && result.Items != null) {
            ...
            //Fire a command to rebind the linked by the async mediator menus
            this._raiseCommandWithArgs(new Telerik.Sitefinity.CommandEventArgs("buildMenu", result));

            if (result.Context) {
                // excecute the command that is specified in the context
                if (result.Context.CommandToExecute && result.Context.OperationName) {
                    this._raiseCommand(result.Context.CommandToExecute, result.Context.OperationName);
                }
                if (!caller._initialBinding && this.get_messageControl()) {
                    if (this._showMessage) {
                        if (result.Context.PositiveMessage) {
                            this.get_messageControl().showPositiveMessage(result.Context.PositiveMessage);
                        }
                        if (result.Context.NegativeMessage) {
                            this.get_messageControl().showNegativeMessage(result.Context.NegativeMessage);
                        }
                    } else {
                        this.get_messageControl().hide();
                    }
                }
                //TODO check if there is a case where we don't want to bind the status field on initial binding as it's required for pages.
                if (this.get_contentWorkflowStatusInfoField()) {
                    this.get_contentWorkflowStatusInfoField().set_dataItemContext(result.Context);
                }
            }
    ...
    // Removes the events from all the previously created elements and clears the containers
    _resetVisuals: function () {
        for (var i = 0, length = this._widgetDomElements.length; i < length; i++) {
            $clearHandlers(this._widgetDomElements[i]);
            // should break the circular reference
    ...
    // Callbakc function called if the workflow service failed to return workflow visuals
    _bindWorkflowVisuals_Failure: function (sender, result) {
        this._afterRequest();
        alert(result.Detail);
    },
    ...
    _messageWorkflow_Success: function (caller, data, request, context) {
        this._afterRequest(false, context);
    ...
    _messageWorkflow_Failure: function (sender, result) {
        ...
        if (!args.get_cancel()) { //when not canceled will continue with the default error handling
            if (sender.Detail) {
                alert(sender.Detail);
            } else {
    ...
    // Generates a DOM element representing a workflow activity based on the WorkflowVisualElement
    // object received from the server
    _generateVisualElement: function (workflowVisualElement) {
        var visualElement = null;
        switch (workflowVisualElement.VisualType) {
            case this._BUTTON:
                visualElement = document.createElement('input');
                $(visualElement).addClass(workflowVisualElement.CssClass);
                if (workflowVisualElement.OperationName != null) {
                    $(visualElement).attr('rel', workflowVisualElement.OperationName);
                }
                ...
                break;
            case this._LINK:
                visualElement = document.createElement('a');
                $(visualElement).addClass(workflowVisualElement.CssClass);
                if (workflowVisualElement.DecisionType === 'MainAction') 
                    $(visualElement).addClass('sfSave');
                visualElement.innerHTML = '<span class="sfLinkBtnIn">' + workflowVisualElement.Title + '</span>';
                if (workflowVisualElement.OperationName != null) {
                    $(visualElement).attr('rel', workflowVisualElement.OperationName);
                    //$(visualElement).attr('href', "javascript:void(0);");
                }
                $addHandler(visualElement, 'click', this._workflowCommandButtonDelegate, true);
                break;
            case this._LABEL:
    ...
    //Calculates an unique id for the element so that it can be searched in the DOM by for example test automation recorders.
    _calculateWorkflowButtonId: function (element, idSuffix) {
        if (idSuffix) {
            jElement = jQuery(element);
            var parentId = "";
            if (jElement.parent() && jElement.parent().attr('id')) {
                parentId = jElement.parent().attr('id') + "_";
            }
            jElement.attr('id', parentId + idSuffix);
        }
    },

    // Handles the command fired by a workflow visual element
    _handleWorkflowButtonCommand: function (sender, args) {

        var relElement = jQuery(sender.target).attr("rel") ? sender.target : sender.target.parentNode;
        var operationName = jQuery(relElement).attr("rel");

        var workflowVisualElement;
        for (var i = 0, length = this._workflowVisualElements.length; i < length; i++) {
            if (this._workflowVisualElements[i].OperationName == operationName) {
                workflowVisualElement = this._workflowVisualElements[i];
                break;
            }
        }
    ...
    _handleWorkflowMenuCommand: function (sender, args) {
        var operationName = args.get_item().get_value();
        if (!operationName) {
            return;
        }
        ...
        if (operationName == "Delete") {
            if (this._preventDeleteParentItem) {
                var cannotDeleteParentDialog = this.getPromptDialogByName("cannotDeleteParentPageDialog");
                if (cannotDeleteParentDialog) {
            ...
            if (this.get_recycleBinEnabled()) {
                deleteMessage = this.get_sendToRecycleBinSingleConfirmationMessage();
            }
    ...
    // handles the response from the warning dialog
    _workflowWarningHandler: function (sender, args) {
    ...
    // when a warning is specified for activity we first show it and then if confirmed we message the workflow
    _excecuteWorkflowCommand: function (workflowVisualElement, operationName) {
        if (workflowVisualElement.WarningMessage) {

            this.get_workflowWarningDialog().show_prompt("",
                                                workflowVisualElement.WarningMessage, null,
                                                { WorkflowVisualElement: workflowVisualElement, OperationName: operationName }
                                            );
        }
        else {
            this._messageWorkflow(workflowVisualElement, operationName);
    ...
    _handleWorkflowMenuMouseOver: function (sender, args) {
        if (args.get_item().get_parent() == sender) {
            sender.set_clicked(false);
        }
    },

    _messageWorkflow: function (workflowVisualElement, operationName, language, checkRelatingData) {
        this._closeOnSuccess = workflowVisualElement.ClosesForm;
        if (workflowVisualElement.ArgumentDialogName != null && workflowVisualElement.ArgumentDialogName != "undefined") {
            if (workflowVisualElement.PersistOnDecision && this._validationFunction && !this._validationFunction()) {
                return;
            }
            ...
            if (workflowVisualElement.ContentCommandName) {
                args += "&contentCommandName=" + workflowVisualElement.ContentCommandName;
            }

            if (this.get_itemId() != Telerik.Sitefinity.getEmptyGuid()) {
                args += "&itemId" + this.get_itemId();
            }

            var dialogName = workflowVisualElement.ArgumentDialogName;
            if (dialogName != null && dialogName != "") {

                var dlg = this._openDialog(this.get_dialogBaseUrl() + dialogName + args, dialogName);
                if (dlg != null) {
                    dlg._sfProperties = workflowVisualElement.Parameters;
                    dlg._sfNoteValue = this._noteValue;
                    // We are checking _validationFunction to see where we are, in content item or page.
                    // _validationFunction not exist in pages.
        ...
        else if (workflowVisualElement.RunAsUICommand) {
            this._raiseCommand(workflowVisualElement.ContentCommandName, "");
        }
        else if (workflowVisualElement.ContentCommandName && workflowVisualElement.PersistOnDecision) {
            this._beforeRequest();
            // if a specific command is secified in the activity - raise it
            this._raiseCommand(workflowVisualElement.ContentCommandName, operationName);
        }
        else // call the WF service to rebind the menu
        {
            var contextBag = null;
            if (language) {
                contextBag = new Array({ key: 'Language', value: language });
            }

            // Add delete related data option in the bag
            if (checkRelatingData) {
                if (!contextBag) {
                    contextBag = new Array();
                }
                contextBag.push({
                    key: "CheckRelatingData",
                    value: checkRelatingData
                });
            }
    ...
    _openDialog: function (url, dialogName) {
    ...
    _getDialogSize: function (dialogName) {
        var dialogSize = { Height: 300, Width: 425 };
        if (dialogName == "WorkflowRejectDialog")
            dialogSize.Height = 400;
        if (dialogName == "WorkflowSendForApprovalDialog")
            dialogSize.Height = 400;

        return dialogSize;
    },

    _raiseCommand: function (contentCommandName, operationName, contextBag) {
        var eventArgs = new Telerik.Sitefinity.CommandEventArgs(contentCommandName, operationName, contextBag);
        return this._raiseCommandWithArgs(eventArgs);
    },

    _raiseCommandWithArgs: function (eventArgs) {
        var h = this.get_events().getHandler('onCommand');
        if (h) h(this, eventArgs);
        return eventArgs;
    },

    _raiseNoWorkflowActions: function (eventArgs) {
        var h = this.get_events().getHandler('onNoWorkflowActions');
        if (h) h(this, eventArgs);
    },

    _raiseOnWorkflowFailure: function (eventArgs) {
        var h = this.get_events().getHandler('onWorkflowFailure');
        if (h) h(this, eventArgs);
    },

    // Bound to the binder via the AsyncMediator
    ...
    // executed when a workflow dialog gets closed
    _endWorkflowWindowClosed: function (sender, args) {
        if (sender != null || sender !== undefined) {
            sender.remove_close(this._workflowWindowCloseDialog);
        }
          
        var args = args.get_argument();
        var persist = sender.AjaxDialog.getQueryValue("PersistOnDecision", true) === "true";
        if (args) {
            var operationName = args.get_operationName();
            var commandName = args.get_commandName();
            var executeCommandOnClose = args.get_executeCommandOnClose();

            if (typeof args.__workflowScheduleDialog != "undefined"
                && args.__workflowScheduleDialog == true) {
                this._dataItem.PublicationDate.constructor = Date;
                if (this._dataItem.ExpirationDate != null) {
                    this._dataItem.ExpirationDate.constructor = Date;
                }
            }

            if (typeof args._noteValue != "undefined" && args._noteValue) {
                this._noteValue = args._noteValue;
            }

            if (persist && !commandName) {
                // defaults to save
                commandName = "save";
            }

            var contextBag = args.get_contextBag();
            if (executeCommandOnClose) {
                this._raiseCommand(commandName, operationName, contextBag);
            }
            else if (operationName) {
                this._sendMessageWorkflow(operationName, contextBag);
            }
            else {
                alert("No content command name or workflow operation specified!")
            }
        }
    },

    // called when the service is called
    _beforeRequest: function (preventRaise, workflowOperation) {
        ...
        if (!preventRaise)
            this._raiseCommand("beforeWMRequest", workflowOperation);
    },

    // called when the service returns result
    _afterRequest: function (preventRaise, workflowOperation) {
        ...
        if (!preventRaise)
            this._raiseCommand("afterWMRequest", workflowOperation);
    },

    _handleWorkflowWindowOpenDialog: function (sender, args) {

        if (sender != null || sender !== undefined) {
            sender.remove_show(this._workflowWindowOpenDialogDelegate);
            sender.remove_pageLoad(this._workflowWindowOpenDialogDelegate)
        }

        if (sender.AjaxDialog) {
            if (sender.AjaxDialog.set_dataItem) {
                sender.AjaxDialog.set_dataItem(this.get_dataItem());
            }
        }
    },
    _onCommand: function (commandName, commandArgument) {

        if (commandName == "beforeWMRequest") {
            this._beforeRequest(true);
        }
        else if (commandName == "afterWMRequest") {
            this._afterRequest(true);
        }
        else if (commandName == "buildMenu" && commandArgument) {
            this._buildMenu(commandArgument.Items, commandArgument.WorkflowDefinition);
        }
    },

    /* ************** properties ********************* */

    get_actionsContainerId: function () {
        return this._actionsContainerId;
    },
    set_actionsContainerId: function (value) {
        this._actionsContainerId = value;
    },


    get_secondaryActionsContainer: function () {
        return this._secondaryActionsContainer;
    },
    set_secondaryActionsContainer: function (value) {
        this._secondaryActionsContainer = value;
    },

    get_actionsContainer: function () {
        if (this._actionsContainer == null) {
            this._actionsContainer = $find(this.get_actionsContainerId());
        }
        return this._actionsContainer;
    },
    set_actionsContainer: function (value) {
        this._actionsContainer = value;
    },
    get_otherActionsMenuId: function () {
        return this._otherActionsMenuId;
    },
    set_otherActionsMenuId: function (value) {
        this._otherActionsMenuId = value;
    },
    get_otherActionsMenu: function () {
        if (this._otherActionsMenu == null) {
            this._otherActionsMenu = $find(this.get_otherActionsMenuId());
        }
        return this._otherActionsMenu;
    },
    set_otherActionsMenu: function (value) {
        this._otherActionsMenu = value;
    },

    // Gets the url of the workflow service
    get_workflowServiceUrl: function () {
        return this._workflowServiceUrl;
    },
    // Sets the url of the workflow service
    set_workflowServiceUrl: function (value) {
        this._workflowServiceUrl = value;
    },

    // Gets  the base dialog url
    get_dialogBaseUrl: function () {
        return this._dialogBaseUrl;
    },
    // Sets the base dialog url
    set_dialogBaseUrl: function (value) {
        this._dialogBaseUrl = value;
    },

    // Gets the id of the workflow item for which the menu is used
    get_workflowItemId: function () {
        return this._workflowItemId;
    },
    // Sets the id of the workflow item for which the menu is used
    ...
    set_loadingView: function (value) {
        this._loadingView = jQuery(value);
    },
    ...
    get_contentWorkflowStatusInfoField: function () {
        return this._contentWorkflowStatusInfoField;
    },
    set_contentWorkflowStatusInfoField: function (value) {
        this._contentWorkflowStatusInfoField = value;
    },
    ...
    // sets culture of the current content item
    set_contentCulture: function (value) {
        this._contentCulture = value;
    },
    // gets culture of the current content item
    get_contentCulture: function () {
        return this._contentCulture;
    },

    get_workflowVisualElements: function () {
        return this._workflowVisualElements;
    },
    // sets the warning dialog 
    set_workflowWarningDialog: function (value) {
        this._workflowWarningDialog = value;
    },
    // gets the warning dialog
    get_workflowWarningDialog: function () {
        return this._workflowWarningDialog;
    },

    // sets the property, indicating if the data item has child nodes
    set_preventDeleteParentItem: function (value) {
        this._preventDeleteParentItem = value;
    },

    get_showMoreActions: function () {
        return this._showMoreActions;
    },
    set_showMoreActions: function (value) {
        this._showMoreActions = value;
    },
    ...
    set_sendToRecycleBinSingleConfirmationMessage: function (value) {
        this._sendToRecycleBinSingleConfirmationMessage = value;
    },
};
...
Telerik.Sitefinity.Workflow.UI.WorkflowMenu.WorkflowFailureEventArgs = function (sender, result) {
    this._sender = sender;
    this._result = result;
    this._cancel = false;
...
Telerik.Sitefinity.Workflow.UI.WorkflowMenu.WorkflowFailureEventArgs.prototype = {

    // ************************** Set up and tear down **************************
    initialize: function () {
        Telerik.Sitefinity.Workflow.UI.WorkflowMenu.WorkflowFailureEventArgs.callBaseMethod(this, 'initialize');
    },
    dispose: function () {
        Telerik.Sitefinity.Workflow.UI.WorkflowMenu.WorkflowFailureEventArgs.callBaseMethod(this, 'dispose');
    },
    // ************************** Properties **************************
    get_sender: function () {
        return this._sender;
