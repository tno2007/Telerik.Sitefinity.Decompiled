...
Telerik.Sitefinity.Modules.Libraries.Web.UI.Designers.DownloadListSelectorDesignerView=function(element){Telerik.Sitefinity.Modules.Libraries.Web.UI.Designers.DownloadListSelectorDesignerView.initializeBase(this,[element]);
...
Telerik.Sitefinity.Modules.Libraries.Web.UI.Designers.DownloadListSelectorDesignerView.prototype={initialize:function(){Telerik.Sitefinity.Modules.Libraries.Web.UI.Designers.DownloadListSelectorDesignerView.callBaseMethod(this,"initialize");
...
},dispose:function(){Telerik.Sitefinity.Modules.Libraries.Web.UI.Designers.DownloadListSelectorDesignerView.callBaseMethod(this,"dispose");
...
}if(this._uploadButtonClickDelegate){if(this._uploadButton){$removeHandler(this._uploadButton,"click",this._uploadButtonClickDelegate);
...
},_resetViewOnProvidersChanged:function(){this._allDocumentsRadio.click();
...
}},_uploadButtonClickHandler:function(e){var dialogManager=GetDialogManager();
...
dialog.Close();
this._isInAdvancedMode=false;
}},_openAlbumSelector:function(){var parentId=this.get_currentMasterView().ItemsParentId;
...
dialogBase.resizeToContent();
},_selectAlbum:function(args){this._dialog.dialog("close");
...
if(selectedTitle.hasOwnProperty("Value")){this.get_selectedLibrary().innerHTML=selectedTitle.Value;
}else{this.get_selectedLibrary().innerHTML=selectedTitle;
...
case this._allDocumentsRadio:$("#selectAlbumWrapper").hide();
$("#uploadWrapper").hide();
break;
case this._uploadRadio:$("#selectAlbumWrapper").hide();
$("#uploadWrapper").show();
break;
}dialogBase.resizeToContent();
},_getCurrentView:function(parent){var settingsDesignerViewId=parent.get_designerViewsMap()["DownloadListSettingsDesignerView"];
...
if(currentView.ItemsParentId==Telerik.Sitefinity.getEmptyGuid()){this._allDocumentsRadio.click();
}else{this._libraryRadio.click();
this.get_selectedLibrary().innerHTML=this._selectedLibraryTitle;
jQuery(this.get_selectedLibrary()).show();
if(this.get_includesChildLibraryItemsLabel()){jQuery(this.get_includesChildLibraryItemsLabel()).toggle(this._includesChildLibraryItems);
}}this._refreshFilterUI();
if(this._sortExpressionChoiceField._get_listItemByValue(currentView.SortExpression).length==0){this._sortExpressionChoiceField.set_value("custom");
}else{this._sortExpressionChoiceField.set_value(currentView.SortExpression);
...
},get_sortExpressionChoiceField:function(){return this._sortExpressionChoiceField;
},set_sortExpressionChoiceField:function(value){this._sortExpressionChoiceField=value;
