...
// Type: Telerik.Sitefinity.Modules.Newsletters.Data.AbTestSendTask
...
{
  public class AbTestSendTask : ScheduledTask
  {
    private Guid abTestId;
    internal const string taskName = "AbTestSendTask";
    private Guid siteId;

    /// <summary>
    /// Initializes a new instance of the <see cref="T:Telerik.Sitefinity.Modules.Newsletters.Data.AbTestSendTask" /> class.
    /// </summary>
    public AbTestSendTask()
    {
    }
    ...
    public AbTestSendTask(Guid abTestId, DateTime executeTime)
    {
    ...
    public override string TaskName => nameof (AbTestSendTask);
    ...
    public override void ExecuteTask()
    {
      using (SiteRegion.FromSiteId(this.siteId))
      {
        ...
        manager.StartTesting(this.abTestId);
        manager.SaveChanges();
      }
    }
    ...
    }) : throw new InvalidOperationException("Cannot schedule an A/B test delivery task without specifying the A/B test id.");
