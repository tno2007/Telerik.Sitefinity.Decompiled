...
Telerik.Sitefinity.Web.UI.RadListViewBinder = function () {
    this._handleItemReordering = false;
...
Telerik.Sitefinity.Web.UI.RadListViewBinder.prototype = {
    // set up and tear down
    initialize: function () {
        this._orginalServiceBaseUrl = this.get_serviceBaseUrl();
        this._containerTag = (this._containerTag != null) ? this._containerTag.toLowerCase() : '';
        if (this._handleItemReordering) {
            ...
            //TODO: expose sortable settings
            var sortableElement = jQuery("#" + this._targetLayoutContainerId);
            sortableElement.sortable({ update: this._sortStopDelegatge,
            ...
            if (this._itemIds != null) {
                for (var i = 0, length = this._itemIds.length; i < length; i++) {
                    var el = sortableElement.children().eq(i);
    ...
    dispose: function () {
        Telerik.Sitefinity.Web.UI.RadListViewBinder.callBaseMethod(this, "dispose");
    ...
    DataBind: function (dataKey) {
        Telerik.Sitefinity.Web.UI.RadListViewBinder.callBaseMethod(this, "DataBind", null);
        var clientManager = this.get_manager();
        clientManager.set_urlParams(this.get_urlParams());

        
        if (dataKey instanceof Array) {
            this.set_globalDataKeys($.extend([], dataKey));
            }
        else
            if (dataKey && dataKey != "") {
                this._serviceBaseUrl = this._serviceChildItemsBaseUrl;
                this.get_globalDataKeys()[this._getItemKeyName()] = dataKey;
            } 
            else {
                this._serviceBaseUrl = this._orginalServiceBaseUrl;
                this.set_globalDataKeys([]);
            }

        if (this._skip == null || this._skip == 0) {
            var listView = $find(this.get_targetId());
    ...
    ReBind: function () {
        ...
        this.DataBind(this.get_globalDataKeys());
    ...
    BindCollection: function (data) {
        ...
        for (var i = 0; i < data.Items.length; i++) {
            ...
            if (this._handleItemReordering) {
                // Keep each dataItem with the corresponding element
                jQuery(itemElement).data("dataItem", dataItem);
            }

            // ensure unique IDs
            $(itemElement).find('[id]').each(function (i1, element) {
    ...
    GetTargetLayoutContainer: function () {
        if (this._targetLayoutContainer)
    ...
    ClearTargetLayoutContainer: function () {
        var target = this.GetTargetLayoutContainer();
        if (target != null) {
    ...
    add_onItemReordered: function (delegate) {
        this.get_events().addHandler('onItemReordered', delegate);
    },
    remove_onItemReordered: function (delegate) {
        this.get_events().removeHandler('onItemReordered', delegate);
    },
    // Returns array with all the elements that are currently bound
    get_currentItemElements: function () {
        return jQuery(this.GetTargetLayoutContainer()).children();
    },

    get_currentItems: function () {
        return this._currentItems;
    },

    selectAll: function () {
        this._selectedItems = [];
        for (var selected = 0; selected < this.get_currentItems().length; selected++) {
            //this._addRemoveSelectedItem(this.get_currentItems()[selected].Id, this.get_currentItems()[selected], "select");
        }
        for (var elementCount = 0; elementCount < this.get_currentItemElements().length; elementCount++) {
            var itemElement = this.get_currentItemElements()[elementCount];
            //            if ($(itemElement).hasClass("sfSel")) {
    ...
    deselectAll: function () {
        ...
        for (var elementCount = 0; elementCount < this.get_currentItemElements().length; elementCount++) {
            ...
            //                $(itemElement).find('.selectCommand:checkbox').attr('checked', 'checked');
            //            }
            //this._itemSelectCommandHandler(this._selectedItems[elementCount].Id, this._selectedItems[elementCount], elementCount, this.get_currentItemElements()[elementCount]);
        }
        for (var selected = 0; selected < this.get_currentItems().length; selected++) {
            //this._addRemoveSelectedItem(this.get_currentItems()[selected].Id, this.get_currentItems()[selected]);
            var key = [];
    ...
    selectByIds: function (ids) {
        ...
        for (var elementCount = 0; elementCount < this.get_currentItemElements().length; elementCount++) {
            ...
            if (ids.indexOf(currentItem.OriginalContentId) >= 0) {
                $(itemElement).removeClass("sfSel");
                var key = [];
    ...
    _onItemReorderedHandler: function (sender, eventArgs) {
        var h = this.get_events().getHandler('onItemReordered');
    ...
    _sortStopHandler: function (event, ui) {
        ...
        var id = el.data("id");
        if (this._autoUpdateOrdinals) {
            this._updateOrdinal(id, oldIndex, newIndex);
        }

        var args = {
            dataItem: el.data("dataItem"),
            oldIndex: oldIndex,
            newIndex: newIndex
        };
    ...
    /* -------------------- private methods ----------- */
    _addRemoveSelectedItem: function (key, dataItem, action) {
        var j = 0;
        if (action == "deselect") {
            while (j < this._selectedItems.length) {
                var key1 = Sys.Serialization.JavaScriptSerializer.serialize(this._selectedItems[j].Key);
    ...
    _updateOrdinal: function (itemId, oldPosition, newPosition) {
    ...
    _saveChangesSuccess: function (caller, result, webRequest) {
        dialogBase.close();
    },
    ...
    ///retrieves the item key name specified
    _getItemKeyName: function () {
        if (this.get_dataKeyNames().length == 0) {
            alert('You must specify a data key name on the RadListViewBinder!');
            return;
        }
        if (this.get_dataKeyNames().length > 1) {
            alert('RadListViewBinder does not support composite keys at this moment.');
            return;
        }
    ...
    set_serviceBaseUrl: function (value) {
        Telerik.Sitefinity.Web.UI.RadListViewBinder.callBaseMethod(this, "set_serviceBaseUrl", [value]);
    ...
    get_selectedItems: function () {
        ...
        for (var idx = 0, lelLength = this._selectedItems.length; idx < lelLength; idx++) {
            dataItems.push(this._selectedItems[idx].Item);
        }
        return dataItems;
    },
