...
Telerik.Sitefinity.Web.UI.Backend.Elements.Widgets.DynamicCommandWidget=function(element){Telerik.Sitefinity.Web.UI.Backend.Elements.Widgets.DynamicCommandWidget.initializeBase(this,[element]);
...
Telerik.Sitefinity.Web.UI.Backend.Elements.Widgets.DynamicCommandWidget.prototype={initialize:function(){Telerik.Sitefinity.Web.UI.Backend.Elements.Widgets.DynamicCommandWidget.callBaseMethod(this,"initialize");
...
switch(this._bindTo){case 0:var select=$get(this._dropDownId);
$addHandler(select,"change",this._handleDropDownChangeDelegate);
break;
case 1:case 2:var moreLink=$get(this._moreLinkId);
if(moreLink!=null){$addHandler(moreLink,"click",this._handleMoreLinkClickDelegate);
}var lessLink=$get(this._lessLinkId);
if(lessLink!=null){$addHandler(lessLink,"click",this._handleLessLinkClickDelegate);
}break;
default:break;
}},dispose:function(){Telerik.Sitefinity.Web.UI.Backend.Elements.Widgets.DynamicCommandWidget.callBaseMethod(this,"dispose");
...
},_loadHandler:function(){var binder=this.getBinder();
switch(this._bindTo){case 0:break;
case 1:binder.add_onItemCommand(this._handleBinderCommandDelegate);
...
break;
case 2:binder.add_onItemCommand(Function.createDelegate(this,this._binderCommandHandler));
binder.add_onDataBound(this._handleBinderDataBoundDelegate);
binder.add_onItemDataBound(this._handleBinderItemDataBoundDelegate);
binder.add_onError(this._errorHandlerDelegate);
break;
}},_moreLinkClicked:function(sender,args){this._currentNumberOfPages++;
this._updateItemsToTake();
},_lessLinkClicked:function(sender,args){this._currentNumberOfPages--;
this._updateItemsToTake();
},_updateItemsToTake:function(sender,args){this.dataBind();
this.preparePager();
},preparePager:function(){var lessLink=$("#"+this._lessLinkId);
var moreLink=$("#"+this._moreLinkId);
var lessLinkExists=lessLink.length>0;
if(lessLinkExists){if(this._currentNumberOfPages==1){lessLink.hide();
}else{lessLink.show();
}}var moreLinkExists=moreLink.length>0;
if(moreLinkExists){if(this._currentNumberOfPages==this._totalNumberOfPages){moreLink.hide();
}else{moreLink.show();
}}},_dropDownChange:function(sender){var selectedValue=$(sender.target+"option:selected").attr("value");
if($.inArray(selectedValue,this._customCommandItemValues)>=0){commandName="dynamicCustomCommand";
}else{commandName=_commandName;
...
}},_binderDataBound:function(sender,args){var moreLink=$("#"+this._moreLinkId);
this._totalItemCount=args.get_dataItem().TotalCount;
this._totalNumberOfPages=Math.ceil(this._totalItemCount/this._pageSize);
var moreLinkExists=moreLink.length>0;
if(this._totalItemCount<=this.get_pageSize()&&moreLinkExists){moreLink.hide();
}if(this._totalItemCount==0){jQuery(this._headerTextControl).hide();
}},_binderCommandHandler:function(sender,args){var commandEventArgs=new Telerik.Sitefinity.UI.CommandEventArgs(this._commandName,args);
commandEventArgs.isFilterCommand=this.get_isFilterCommand();
this.set_selectedItemId(args.get_dataItem().Id);
var h=this.get_events().getHandler("command");
if(h){h(this,commandEventArgs);
}},_binderItemDataBound:function(sender,args){var dataItem=args.get_dataItem();
if(dataItem.Id==this._selectedItemId){$(args.get_itemElement()).find("a").addClass("sfSel");
...
}if(this._pageSize==0){this.get_urlParameters()["take"]=this._pageSize;
this.preparePager();
}else{this.get_urlParameters()["take"]=this._currentNumberOfPages*this.get_pageSize();
}this.getBinder().set_urlParams(this.get_urlParameters());
if(this._bindTo===1||this._bindTo===2){var binder=this.getBinder();
var context=this.get_selectedItemId();
if(context){binder.DataBind(null,context);
}else{binder.DataBind();
}}this._isBound=true;
},chnageVisibility:function(visible){this._hidden=!visible;
if(visible){if(!this._isBound){this.dataBind();
}}else{this.set_selectedItemId(null);
this._isBound=false;
...
},set_hidden:function(value){if(this._hidden!=value){this._hidden=value;
...
},set_urlParameters:function(value){if(this._urlParameters!=value){this._urlParameters=value;
this.raisePropertyChanged("urlParameters");
}},get_selectedItemId:function(){return this._selectedItemId;
},set_selectedItemId:function(value){if(this._selectedItemId!=value){this._selectedItemId=value;
...
},get_headerTextControl:function(){return this._headerTextControl;
},set_headerTextControl:function(value){this._headerTextControl=value;
},get_customCommandItemValues:function(){return this._customCommandItemValues;
},set_customCommandItemValues:function(value){this._customCommandItemValues=value;
},get_customCommandItemTexts:function(){return this._customCommandItemTexts;
},set_customCommandItemTexts:function(value){this._customCommandItemTexts=value;
