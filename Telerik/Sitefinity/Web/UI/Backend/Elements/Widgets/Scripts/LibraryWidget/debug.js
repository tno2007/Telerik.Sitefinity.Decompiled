...
Telerik.Sitefinity.Web.UI.Backend.Elements.Widgets.LibraryWidget = function (element) {
    Telerik.Sitefinity.Web.UI.Backend.Elements.Widgets.LibraryWidget.initializeBase(this, [element]);
...
Telerik.Sitefinity.Web.UI.Backend.Elements.Widgets.LibraryWidget.prototype = {

    initialize: function () {
        Telerik.Sitefinity.Web.UI.Backend.Elements.Widgets.LibraryWidget.callBaseMethod(this, 'initialize');
        Sys.Application.add_load(Function.createDelegate(this, this._loadHandler));
    },

    dispose: function () {
        Telerik.Sitefinity.Web.UI.Backend.Elements.Widgets.LibraryWidget.callBaseMethod(this, 'dispose');
    },

    _loadHandler: function (sender, args) {
        this.get_binder().add_onItemDataBound(Function.createDelegate(this, this._binderItemDataBoundHandler));
    ...
    _binderItemDataBoundHandler: function (sender, args) {
        var jItemElement = $(args.get_itemElement());
        var itemsText = jItemElement.find(".sfLibItemsText").get(0);
        if (args.get_dataItem().ItemsCount == 1) {
            itemsText.text = this._itemName;
            itemsText.innerHTML = this._itemName;
        }
        else {
            itemsText.text = this._itemsName;
        ...
        jQuery.each(otherActionMenuItems, function () {
            if (jQuery(this).hasClass("sf_binderCommand_reorder")) {
                if (supportsReordering) {
                    this.innerHTML += itemsName;
                }
                else {
                    jQuery(this).hide();
                }
                ...
                this.innerHTML = String.format(this.innerHTML, libraryName);
            }
        });
    ...
    _binderItemCommandHandler: function (sender, args) {
        var commandEventArgs = new Telerik.Sitefinity.UI.CommandEventArgs(args.get_commandName(), args.get_dataItem());
    ...
    get_supportsReordering: function () {
        return this._supportsReordering;
    },
    set_supportsReordering: function (value) {
        this._supportsReordering = value;
    }
}
