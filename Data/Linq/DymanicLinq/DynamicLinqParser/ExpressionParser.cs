...
// Type: Telerik.Sitefinity.Data.Linq.Dynamic.ExpressionParser
...
namespace Telerik.Sitefinity.Data.Linq.Dynamic
{
  ...
  {
    private static readonly Type[] predefinedTypes = new Type[21]
    {
      typeof (object),
      typeof (bool),
      typeof (char),
    ...
    private Type leftEnumType;
    private MethodInfo _propertyValueMethodInfo;

    public ExpressionParser(ParameterExpression[] parameters, string expression, object[] values)
      : this((Type) null, parameters, expression, values)
    {
    }
    ...
    private void ProcessParameters(ParameterExpression[] parameters)
    {
      ...
      if (parameters.Length != 1 || !string.IsNullOrEmpty(parameters[0].Name) && !(parameters[0].Name == "item"))
        return;
      this.it = parameters[0];
      this.currentParameter = parameters[0];
    }
    ...
    public Expression Parse(Type resultType)
    {
      ...
      if (resultType != (Type) null && (expression2 = this.PromoteExpression(expression1, resultType, true)) == null)
    ...
    private Expression ParseLogicalOr()
    {
      ...
      while (this.token.id == ExpressionParser.TokenId.DoubleBar || this.TokenIdentifierIs("or"))
      {
        ...
        this.CheckAndPromoteOperands(typeof (ILogicalSignatures), token.text, ref left, ref logicalAnd, token.pos);
    ...
    private Expression ParseLogicalAnd()
    {
      ...
      while (this.token.id == ExpressionParser.TokenId.DoubleAmphersand || this.TokenIdentifierIs("and"))
      {
        ...
        this.CheckAndPromoteOperands(typeof (ILogicalSignatures), token.text, ref left, ref comparison, token.pos);
    ...
    private Expression ParseComparison()
    {
      ...
      Type type = left.Type;
      if (left.NodeType == ExpressionType.MemberAccess && type.IsEnum)
        this.leftEnumType = type;
      while (this.token.id == ExpressionParser.TokenId.Equal || this.token.id == ExpressionParser.TokenId.DoubleEqual || this.token.id == ExpressionParser.TokenId.ExclamationEqual || this.token.id == ExpressionParser.TokenId.LessGreater || this.token.id == ExpressionParser.TokenId.GreaterThan || this.token.id == ExpressionParser.TokenId.GreaterThanEqual || this.token.id == ExpressionParser.TokenId.LessThan || this.token.id == ExpressionParser.TokenId.LessThanEqual)
      {
        ...
        if (flag && !left.Type.IsValueType && (!right.Type.IsValueType || right.Type.IsValueType && left.Type == typeof (object)))
        {
          if (left.Type != right.Type)
          {
            if (left.Type != typeof (object) && left.Type != typeof (object))
            {
    ...
    private void FixExpressions(ref Expression left, Expression right)
    {
      ...
      while (this.token.id == ExpressionParser.TokenId.Plus || this.token.id == ExpressionParser.TokenId.Minus || this.token.id == ExpressionParser.TokenId.Amphersand)
      {
        ...
        switch (token.id)
        {
          ...
          case ExpressionParser.TokenId.Plus:
            ...
            {
              this.CheckAndPromoteOperands(typeof (IAddSignatures), token.text, ref left, ref multiplicative, token.pos);
          ...
          case ExpressionParser.TokenId.Minus:
            this.CheckAndPromoteOperands(typeof (ISubtractSignatures), token.text, ref left, ref multiplicative, token.pos);
    ...
    private Expression ParseMultiplicative()
    {
      ...
      while (this.token.id == ExpressionParser.TokenId.Asterisk || this.token.id == ExpressionParser.TokenId.Slash || this.token.id == ExpressionParser.TokenId.Percent || this.TokenIdentifierIs("mod"))
      {
        ...
        this.CheckAndPromoteOperands(typeof (IArithmeticSignatures), token.text, ref left, ref unary, token.pos);
        switch (token.id)
        {
    ...
    private Expression ParseUnary()
    {
      ...
      {
        this.CheckAndPromoteOperand(typeof (INegationSignatures), token.text, ref unary1, token.pos);
        unary2 = (Expression) Expression.Negate(unary1);
      }
      else
      {
        this.CheckAndPromoteOperand(typeof (INotSignatures), token.text, ref unary1, token.pos);
    ...
    private Expression ParsePrimaryStart()
    {
      switch (this.token.id)
      {
        ...
        case ExpressionParser.TokenId.Guid:
          return this.ParseGuidLiteral();
        case ExpressionParser.TokenId.DateTime:
    ...
    {
      this.ValidateToken(ExpressionParser.TokenId.DateTime);
      if (this.token.text.Contains<char>('('))
        this.token.text = this.token.text.Replace("(", "");
      if (this.token.text.Contains<char>(')'))
        this.token.text = this.token.text.Replace(")", "");
      DateTime dateTime = DateTime.Parse(this.token.text.Trim().Replace("UTC", "GMT"), (IFormatProvider) CultureInfo.InvariantCulture.DateTimeFormat, DateTimeStyles.AdjustToUniversal | DateTimeStyles.AssumeUniversal);
      this.NextToken();
      return this.CreateLiteral((object) dateTime, dateTime.ToString("yyyy-MM-dd hh:mm:ss"));
    }

    private Expression ParseGuidLiteral()
    {
      this.ValidateToken(ExpressionParser.TokenId.Guid);
      Guid guid = new Guid(this.token.text.Replace("\"", ""));
      this.NextToken();
      return this.CreateLiteral((object) guid, guid.ToString());
    }
    ...
    private Expression ParseStringLiteral()
    {
      ...
      if (ch == '\'')
      {
        ...
        this.NextToken();
        return this.CreateLiteral((object) (int) text[0], text);
      }
      this.NextToken();
    ...
    private Expression ParseIntegerLiteral()
    {
      ...
      {
        ulong result;
        if (!ulong.TryParse(text, NumberStyles.Any, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result))
          throw this.ParseError("Invalid integer literal '{0}'", (object) text);
        this.NextToken();
        if (this.token.id == ExpressionParser.TokenId.Identifier && (this.token.text == "L" || this.token.text == "l"))
        {
          this.NextToken();
          return this.CreateLiteral((object) long.Parse(text, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat), text);
        }
        if (this.token.id == ExpressionParser.TokenId.Identifier && (this.token.text == "u" || this.token.text == "U"))
        {
          this.NextToken();
          return this.CreateLiteral((object) uint.Parse(text, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat), text);
        }
        if (this.token.id == ExpressionParser.TokenId.Identifier && (this.token.text == "ul" || this.token.text == "UL" || this.token.text == "ul" || this.token.text == "Ul" || this.token.text == "uL" || this.token.text == "LU" || this.token.text == "lu" || this.token.text == "Lu" || this.token.text == "lU"))
        {
          this.NextToken();
          return this.CreateLiteral((object) ulong.Parse(text, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat), text);
        }
      ...
      long result1;
      if (!long.TryParse(text, NumberStyles.Any, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result1))
        throw this.ParseError("Invalid integer literal '{0}'", (object) text);
      this.NextToken();
      if (this.token.id == ExpressionParser.TokenId.Identifier && (this.token.text == "L" || this.token.text == "l"))
      {
    ...
    private Expression ParseRealLiteral()
    {
      ...
      switch (text[text.Length - 1])
      {
        ...
        case 'f':
          float result1;
          if (float.TryParse(text.Substring(0, text.Length - 1), NumberStyles.Any, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result1))
          {
            obj = (object) result1;
            break;
          }
        ...
        case 'm':
          Decimal result2;
          if (Decimal.TryParse(text.Substring(0, text.Length - 1), NumberStyles.Any, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result2))
          {
            obj = (object) result2;
            break;
          }
        ...
        default:
          double result3;
          if (double.TryParse(text, NumberStyles.Any, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result3))
          {
            obj = (object) result3;
            break;
          }
    ...
    private Expression ParseIdentifier()
    {
      ...
      {
        if ((object) (identifier1 as Type) != null)
      ...
      if (this.ParseEnumType(out identifier1))
      {
        ConstantExpression identifier3 = Expression.Constant(identifier1);
        this.NextToken();
        return (Expression) identifier3;
      }
    ...
    private bool ParseEnumType(out object value)
    {
      value = (object) null;
      this.ValidateToken(ExpressionParser.TokenId.Identifier);
      if (this.leftEnumType == (Type) null)
        return false;
      try
      {
        value = Enum.Parse(this.leftEnumType, this.token.text, true);
      }
      catch
      {
        this.leftEnumType = (Type) null;
        return false;
      }
      this.leftEnumType = (Type) null;
      return true;
...
label_8:
    ...
    private Expression GenerateConversion(Expression expr, Type type, int errorPos)
    {
      ...
      {
        if ((type1.IsNullableType() || type.IsNullableType()) && type1.GetNonNullableType() == type.GetNonNullableType())
    ...
    private Expression ParsePropertyMethodCall(
      Type type,
      string methodName,
      Expression currentExpression,
      Expression[] arguments)
    {
      return (Expression) Expression.Constant((object) type);
    }
    ...
    private Expression ParseMemberAccess(Type type, Expression instance)
    {
      ...
      if (this.token.id == ExpressionParser.TokenId.OpenParen)
      {
        if (instance != null && !(type == typeof (string)) && !(type.Name == "Lstring"))
        {
          Type genericType = ExpressionParser.FindGenericType(typeof (IEnumerable<>), type);
          if (genericType != (Type) null && ((IEnumerable<MethodInfo>) genericType.GetMethods()).Where<MethodInfo>((Func<MethodInfo, bool>) (u => u.Name == id)).Any<MethodInfo>())
          {
            Type genericArgument = genericType.GetGenericArguments()[0];
            return this.ParseAggregate(instance, genericArgument, id, pos);
          }
          IEnumerable<MethodInfo> source = ((IEnumerable<MethodInfo>) type.GetMethods()).Where<MethodInfo>((Func<MethodInfo, bool>) (u => u.Name == id));
          if (source.Count<MethodInfo>() > 0)
          {
            Expression[] arguments = this.ParseArgumentList();
            MethodInfo method = source.Where<MethodInfo>((Func<MethodInfo, bool>) (u => u.GetParameters().Length == arguments.Length)).FirstOrDefault<MethodInfo>();
            if (method == (MethodInfo) null)
              method = source.FirstOrDefault<MethodInfo>();
            if (!ExpressionParser.IsPredefinedType(method.DeclaringType))
              throw this.ParseError(pos, "Methods on type '{0}' are not accessible", (object) ExpressionParser.GetTypeName(method.DeclaringType));
            return (Expression) Expression.Call(instance, method, ((IEnumerable<Expression>) arguments).ToArray<Expression>());
          }
          if (type.IsArray && ((IEnumerable<MethodInfo>) typeof (Enumerable).GetMethods()).Where<MethodInfo>((Func<MethodInfo, bool>) (mi => mi.IsDefined(typeof (ExtensionAttribute), false))).Any<MethodInfo>((Func<MethodInfo, bool>) (mi => mi.Name == id)))
          {
            Type genericArgument = genericType.GetGenericArguments()[0];
            return this.ParseAggregate(instance, genericArgument, id, pos);
          }
        ...
        Expression[] argumentList = this.ParseArgumentList();
        MethodBase method1;
        int method2 = this.FindMethod(type, id, instance == null, argumentList, out method1);
        if (type == typeof (object) && method2 == 0)
        {
          CSharpArgumentInfo csharpArgumentInfo1 = CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, (string) null);
          CSharpArgumentInfo csharpArgumentInfo2 = CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.UseCompileTimeType | CSharpArgumentInfoFlags.Constant, (string) null);
          CallSiteBinder binder = Microsoft.CSharp.RuntimeBinder.Binder.InvokeMember(CSharpBinderFlags.None, id, (IEnumerable<Type>) null, type, (IEnumerable<CSharpArgumentInfo>) new CSharpArgumentInfo[2]
          {
            csharpArgumentInfo1,
            csharpArgumentInfo2
          });
          ...
          expressionList.AddRange((IEnumerable<Expression>) argumentList);
          Type returnType = type;
          Expression[] array = expressionList.ToArray();
          return (Expression) Expression.Dynamic(binder, returnType, array);
        }
        switch (method2)
        {
          case 0:
            throw this.ParseError(pos, "No applicable method '{0}' exists in type '{1}'", (object) id, (object) ExpressionParser.GetTypeName(type));
          case 1:
            MethodInfo method3 = (MethodInfo) method1;
            if (!ExpressionParser.IsPredefinedType(method3.DeclaringType))
              throw this.ParseError(pos, "Methods on type '{0}' are not accessible", (object) ExpressionParser.GetTypeName(method3.DeclaringType));
            if (method3.ReturnType == typeof (void))
              throw this.ParseError(pos, "Method '{0}' in type '{1}' does not return a value", (object) id, (object) ExpressionParser.GetTypeName(method3.DeclaringType));
            return (Expression) Expression.Call(instance, method3, argumentList);
          default:
            throw this.ParseError(pos, "Ambiguous invocation of method '{0}' in type '{1}'", (object) id, (object) ExpressionParser.GetTypeName(type));
        }
      }
      else
      {
        Expression expression = (Expression) null;
        MemberInfo propertyOrField = this.FindPropertyOrField(type, id, instance == null);
        if (propertyOrField == (MemberInfo) null)
        {
          if (typeof (IDynamicFieldsContainer).IsAssignableFrom(type))
            expression = this.CreateMemberAccessExpressionOverride(type, id, instance) ?? this.CreateMemberAccessExpressionOverride(this.dynamicType, id, instance);
          if (typeof (WrapperObject).IsAssignableFrom(type))
            expression = this.CreateMemberAccessExpressionWrapperOverride(type, id, instance);
          if (expression == null)
            throw new MemberAccessException(string.Format("No '{0}' member found in type '{1}'", (object) id, (object) type.FullName));
        }
        else
          expression = (object) (propertyOrField as PropertyInfo) != null ? (Expression) Expression.Property(instance, (PropertyInfo) propertyOrField) : (Expression) Expression.Field(instance, (FieldInfo) propertyOrField);
        if (expression != null && expression.Type == typeof (char))
          expression = (Expression) Expression.Convert(expression, typeof (int));
        if (expression != null && (expression.Type == typeof (Lstring) || expression.Type == typeof (ChoiceOption)) && this.token.id != ExpressionParser.TokenId.OpenBracket)
      ...
      int errorPos)
    {
      ...
      this.it = it;
      MethodBase method;
      if (this.FindMethod(typeof (IEnumerableSignatures), methodName, false, argumentList, out method) != 1)
    ...
    {
      if (type.IsGenericType && ((IEnumerable<Type>) type.GetInterfaces()).Where<Type>((Func<Type, bool>) (i => i.IsGenericType)).Any<Type>((Func<Type, bool>) (i => i.GetGenericTypeDefinition() == typeof (IEnumerable<>))))
      {
        Type[] genericArguments = type.GetGenericArguments();
        return genericArguments.Length <= 1 ? ExpressionParser.IsPredefinedType(genericArguments[0]) : throw new NotSupportedException("Collections with more than one generic argument are not supported.");
      }
      if (type.Name == "Lstring")
        return true;
    ...
    private static bool IsNumericType(Type type) => (uint) ExpressionParser.GetNumericTypeKind(type) > 0U;
    ...
    private static bool IsEnumType(Type type) => type.GetNonNullableType().IsEnum;
    ...
    private MethodInfo WrappedValueMethodInfo
    {
      get
      {
        if (this._propertyValueMethodInfo == (MethodInfo) null)
          this._propertyValueMethodInfo = ((IEnumerable<MethodInfo>) typeof (WrapperObjectExtensions).GetMethods(BindingFlags.Static | BindingFlags.Public)).Where<MethodInfo>((Func<MethodInfo, bool>) (u => u.Name == "GetWrapperValue")).First<MethodInfo>();
        return this._propertyValueMethodInfo;
      }
    }

    private MethodInfo PropertyValueMethodInfo
    {
      get
      {
        if (this._propertyValueMethodInfo == (MethodInfo) null)
          this._propertyValueMethodInfo = ((IEnumerable<MethodInfo>) typeof (DataExtensions).GetMethods(BindingFlags.Static | BindingFlags.Public)).Where<MethodInfo>((Func<MethodInfo, bool>) (u => u.Name == "GetValue" && u.IsGenericMethod)).First<MethodInfo>();
        return this._propertyValueMethodInfo;
      }
    }

    protected Expression CreateMemberAccessExpressionOverride(
      Type itemType,
      string memberName,
      Expression instance)
    {
      ...
      PropertyDescriptor propertyDescriptor = TypeDescriptor.GetProperties(itemType).Find(memberName, true);
      if (propertyDescriptor == null)
        return (Expression) null;
      ConstantExpression constantExpression = Expression.Constant((object) propertyDescriptor.Name);
      Type type = propertyDescriptor.PropertyType == typeof (ChoiceOption[]) ? typeof (string[]) : propertyDescriptor.PropertyType;
      ParameterExpression currentParameter = this.currentParameter;
      Expression expressionOverride;
      if (instance.NodeType == ExpressionType.Call && ((MethodCallExpression) instance).Method.Name == this.PropertyValueMethodInfo.Name)
        expressionOverride = (Expression) Expression.Call(this.PropertyValueMethodInfo.MakeGenericMethod(type), instance, (Expression) constantExpression);
      else
        expressionOverride = (Expression) Expression.Call(this.PropertyValueMethodInfo.MakeGenericMethod(type), (Expression) currentParameter, (Expression) constantExpression);
      return expressionOverride;
    }

    protected Expression CreateMemberAccessExpressionWrapperOverride(
      Type itemType,
      string memberName,
      Expression instance)
    {
      if (itemType == (Type) null)
        return (Expression) null;
      ConstantExpression constantExpression = Expression.Constant((object) memberName);
      ParameterExpression currentParameter = this.currentParameter;
      return instance.NodeType != ExpressionType.Call || !(((MethodCallExpression) instance).Method.Name == this.WrappedValueMethodInfo.Name) ? (Expression) Expression.Call(this.WrappedValueMethodInfo, (Expression) currentParameter, (Expression) constantExpression) : (Expression) Expression.Call(this.WrappedValueMethodInfo, instance, (Expression) constantExpression);
    }
    ...
    private Expression PromoteExpression(Expression expr, Type type, bool exact)
    {
      ...
      if (!ExpressionParser.IsCompatibleWith(expr, type))
        return (Expression) null;
    ...
    private static object ParseNumber(string text, Type type)
    {
      switch (Type.GetTypeCode(type.GetNonNullableType()))
      {
        case TypeCode.SByte:
          sbyte result1;
          if (sbyte.TryParse(text, NumberStyles.Any, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result1))
            return (object) result1;
          break;
        case TypeCode.Byte:
          byte result2;
          if (byte.TryParse(text, NumberStyles.Any, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result2))
            return (object) result2;
          break;
        case TypeCode.Int16:
          short result3;
          if (short.TryParse(text, NumberStyles.Any, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result3))
            return (object) result3;
          break;
        case TypeCode.UInt16:
          ushort result4;
          if (ushort.TryParse(text, NumberStyles.Any, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result4))
            return (object) result4;
          break;
        case TypeCode.Int32:
          int result5;
          if (int.TryParse(text, NumberStyles.Any, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result5))
            return (object) result5;
          break;
        case TypeCode.UInt32:
          uint result6;
          if (uint.TryParse(text, NumberStyles.Any, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result6))
            return (object) result6;
          break;
        case TypeCode.Int64:
          long result7;
          if (long.TryParse(text, NumberStyles.Any, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result7))
            return (object) result7;
          break;
        case TypeCode.UInt64:
          ulong result8;
          if (ulong.TryParse(text, NumberStyles.Any, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result8))
            return (object) result8;
          break;
        case TypeCode.Single:
          float result9;
          if (float.TryParse(text, NumberStyles.Any, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result9))
            return (object) result9;
          break;
        case TypeCode.Double:
          double result10;
          if (double.TryParse(text, NumberStyles.Any, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result10))
            return (object) result10;
          break;
        case TypeCode.Decimal:
          Decimal result11;
          if (Decimal.TryParse(text, NumberStyles.Any, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result11))
    ...
    private static bool IsCompatibleWith(Expression source, Type target)
    {
      if (source.Type == typeof (object))
        source = (Expression) Expression.Convert(source, target);
      return source.NodeType == ExpressionType.Call ? ExpressionParser.IsCompatibleWith(source.Type, target) : ExpressionParser.IsCompatibleWith(source.Type, target);
    }
    ...
    {
      this.FixExpressions(ref left, right);
    ...
    private void NextToken()
    {
      ...
      while (!char.IsWhiteSpace(this.ch) && this.textPos < this.textLen && this.ch != ')')
      {
        stringBuilder1.Append(this.ch);
        this.NextChar();
      }
      ExpressionParser.TokenId tokenId;
      if (stringBuilder1.Length >= 32 && Guid.TryParse(stringBuilder1.ToString(), out Guid _))
      {
        tokenId = ExpressionParser.TokenId.Guid;
      }
      else
      {
        if (this.ch == ')')
          stringBuilder1.Append(this.ch);
        this.ch = ch1;
        this.textPos = textPos2;
        StringBuilder stringBuilder2 = new StringBuilder();
        char ch2 = this.ch;
        int textPos3 = this.textPos;
        StringBuilder stringBuilder3 = new StringBuilder();
        while (!char.IsWhiteSpace(this.ch) && this.textPos < this.textLen && stringBuilder3.Length <= 38)
        {
          stringBuilder3.Append(this.ch);
          this.NextChar();
        }
        if (stringBuilder3.Length >= 32)
        {
          if (Guid.TryParse(stringBuilder3.ToString().Replace("\"", ""), out Guid _))
          {
            tokenId = ExpressionParser.TokenId.Guid;
            this.NextChar();
            goto label_89;
        ...
        if (this.ch == '(')
        {
          ...
          this.NextChar();
          while (this.textPos < this.textLen && this.ch != ')')
          {
            stringBuilder4.Append(this.ch);
            this.NextChar();
          }
          DateTime result = new DateTime();
          if (DateTime.TryParse(stringBuilder4.ToString().Replace("UTC", "GMT"), (IFormatProvider) CultureInfo.InvariantCulture.DateTimeFormat, DateTimeStyles.None, out result))
          {
            tokenId = ExpressionParser.TokenId.DateTime;
            this.NextChar();
            goto label_89;
          }
          else
            this.SetTextPos(textPos4);
        }
        switch (this.ch)
        {
          case '!':
          ...
          case '\'':
            ...
            {
              this.NextChar();
              while (this.textPos < this.textLen && (int) this.ch != (int) ch3)
                this.NextChar();
              if (this.textPos == this.textLen)
                throw this.ParseError(this.textPos, "Unterminated string literal");
              this.NextChar();
            }
            while ((int) this.ch == (int) ch3);
          ...
          default:
            ...
            if (char.IsDigit(this.ch))
            {
              ...
              if (this.ch == 'F' || this.ch == 'f' || this.ch == 'm' || this.ch == 'M')
              {
                tokenId = ExpressionParser.TokenId.RealLiteral;
                this.NextChar();
                break;
